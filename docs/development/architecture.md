...
app/platform/app.py
  ↓
app/platform/router.py        # 产生候选 Skill
  ↓
app/platform/task_relations.py # 确定目标任务和关系
  ↓
...
- `app/platform/task_relations.py`：跨 Skill 任务关系层，按入口和 `userid` 维护多任务卡片、版本、材料角色、父子任务和待确认状态；固定区分续改、补料、派生、新建、回答追问、切换、取消和需要确认。目标选择依次尊重任务序号、明确标题或文种、用户当前稿件；“再改、继续改、接着改、这个直报/简报”以及对当前稿“输出/导出 Word、生成正式文档”等承接表达直接落到当前稿，不交给模型按主题猜测。剩余模糊场景才调用结构化模型，高置信度结果仍须通过目标任务和状态硬校验。
- `app/platform/revision.py`：跨写作 Skill 的公共改稿执行引擎。它把自然语言要求转换为统一 `RevisionPlan`，确定标题、指定段落或全文范围，在模型生成后恢复未点名内容，并检查篇幅、数字保留和指定删除项；不包含直报、简报或润色的文种规则。
...
- `app/writing/task_execution.py`：直报、简报、综合调研、深银协动态和内参周报的持久任务适配层；简报只使用 `writer1` 一个队列 ID，入队前创建正式写作 job 和文件快照，后台按“处理、会话收尾、主动发送”三个检查点恢复。直报或简报生成正式 Word 时，标题正文文字和任务 `output/` 内 `.docx` 附件作为两个独立交付项发送和恢复；综合调研按完成提示和 `.docx` 初稿分别交付；内参周报初次生成按“完成说明、精简 Markdown 核对稿”交付，JSON 溯源记录仅在任务内部保存，人工批准当前版本后按“完成说明、洁净版 Word”交付。每项主动发送结果还写入不含正文、文件名、路径和用户 ID 的状态日志。
- 综合调研、深银协动态和内参周报的附件属于成功结果必备项。缺失、空文件、后缀不符或超出本次任务 `output/` 时，持久任务按处理失败重试并在耗尽后告警，不能退化为只发“已完成”文字；直报和简报按用户要求允许纯文字结果。
...
- 写作和独立润色入口已接入任务关系层。同一用户可保留并定位多篇稿件；追问后可沿用原文字、链接和文件继续。审核入口继续使用自己的任务组装状态机，已入队审核的追加、替换和切换仍由审核专项治理。
...
- 后台任务执行器已完成生产竞态加固。纯文字、单个通用 Word、内参、半月报、公文格式、单个静态 HTML、单份 PPTX 和多文件联合审核接入审核专用持久队列；直报、`writer1`、`research_synthesis`、`shenyinxie_news` 和 `internal_weekly` 接入独立写作队列。
...
- **已经进入切流验收**：审核八类能力使用审核专用 SQLite；直报、简报、综合调研、深银协动态和内参周报使用写作专用 SQLite。十三类 handler 都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态按统一三态保存，无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。综合调研的完成提示和 Word、内参周报核对稿的 Markdown、JSON 以及批准后的 Word 分别按实际输出记录检查点，任一已经确认送达的交付项不会因恢复重复发送。

当前入口接入边界：

- 审核八类能力使用审核专用队列，直报、writer1、research_synthesis、shenyinxie_news 和 internal_weekly 使用写作专用队列。
- `rewrite` 属于即时多轮交互，不作为当前后台初稿队列任务。
...
业务 Skill 路由仍只能从注册表和权限范围内选择能力。路由前先经过 `app/platform/task_relations.py`：它先回答“这条消息与哪项任务是什么关系”，再把已定位的 Skill 和材料角色交给业务 workflow。旧 `conversation.py` 继续保存兼容会话和版本记录，但不再作为多任务定位的唯一事实来源。

任务卡片按 `channel + userid` 隔离，保存标题、Skill、状态、当前版本、结构摘要、材料台账、待回答问题和父任务。最近时间只作辅助；用户点名标题、序号或上一轮问题时优先采用明确证据。多个任务都可能匹配时，系统保存原要求并只问一个区分问题；用户回答后恢复原文字、链接和文件，不创建无关空任务。

模糊关系使用与写作相同的 Pydantic AI 模型返回固定结构。模型最多看到当前用户 8 项任务的受限摘要，不接收其他用户任务、完整历史正文或本机路径。目标任务不存在、状态不允许或置信度低于 0.78 时，代码拒绝自动执行并追问。关系库只记录不含消息正文的聚合判断指标，供前期校准追问率、恢复率和用户纠正率。

直报和 `writer1` 的普通续改、补充/替换/参考材料已经进入写作持久队列。对当前直报或简报只要求“导出 Word”时，任务关系层按继续当前稿执行，Skill 直接复用上一稿内容，不调用写作模型；直报期号、总期号和落款日期等明确版头要求也按同一方式续接。简报明确修改日期、期号、责任编辑或联系电话时，工作流复用上一稿和已有文档字段直接重新生成 Word，签发人锁定为母版值。目标稿本身仍在生成时先保存要求并提示等待；不同稿件可继续排队，同一用户执行并发仍为 1，不同用户由队列并发上限共同调度。沿用旧任务结构另写一份会建立父子任务关系，只传递结构摘要，不自动混入旧稿事实。

改稿执行层还有三条硬约束：

- `previous_draft` 类型材料在模型 prompt 中保留更长文本，避免长简报只传入前半部分，导致模型看不到后续段落。
- `app/platform/revision.py` 先用公共结构化模型识别改稿动作，模型不可用或结果无效时退回本地确定性解析；“只改标题”和指定段落由代码恢复未点名内容，不能只依靠写作 prompt。
- 公共引擎生成后执行确定性自检；篇幅、数字保留或指定删除项未满足时只允许一次带反馈重写，第二次仍未通过则明确提示人工复核。`revision_plan` 经过字段白名单清洗后随持久任务结果保存，但不进入普通企业微信回复。
- 平台结果和会话版本可透传受限 `document_metadata`；简报 Skill 只允许期号、日期、责任编辑和联系电话四个键，并在调用写作模型前移除这些版式指令，避免进入正文。历史版本没有该字段时按空值读取。
...
BriefResult / BriefPlanResult / BriefCriticResult
app/platform/revision.py
RevisionPlan / RevisionViolation
```
...
当前登记的业务能力包括直报、统一简报、材料润色、综合调研整合、深银协动态和内参周报。简报以 `skills/writer1/` 作为唯一规则、提示词、workflow、权限和队列来源；历史持久化数据中的退役 ID 只在读取边界归一，不登记为 Skill。直报、简报和材料润色都通过 `RevisionEngine` 执行续改，各 Skill 只配置是否有标题和合理篇幅范围，并继续独立维护文种规则、质量检查和 workflow；新增支持改稿的 Skill 不得复制一套改稿解析器。所有 Skill 都只能调用底座授权工具。内参周报内容生产与 `app/review/` 的内参审核、半月报审核相互独立；分类规则复制到 Skill 自有目录后维护，运行时不依赖审核模块。该 Skill 先交付可追溯核对稿，只有用户明确批准同一 `draft_version` 后才复制净化母版生成 Word；批准后导出复用任务关系层保存的当前版本正文和受限文档元数据，不重新检索或调用模型。
...
- 用分层架构图展示“入口 -> 受控 Agent 内核 -> 写作/审核 -> 确认交付”的主干流程，以及运行约束、共享工具知识和治理控制面；用独立状态清单展示各功能和模块的建设状态。
...
1. 架构图说明系统如何工作，并展示全部已登记架构节点：业务主链、底座的接入/编排/运行三层、写作和审核子能力、受限工具、知识资产及治理控制面。七条主干关系按从上到下的顺序绘制，普通阶段使用垂直直线，写作/审核分流和结果汇合使用圆角转弯线；其他依赖通过固定分区和层级表达。写作与审核分别使用绿色和蓝色，身份隔离、工具白名单、结构化输出、幂等恢复和交付确认作为贯穿任务全程的五道约束单独呈现。
...
状态清单把建设成熟度、运行在线状态和执行方式分开：成熟度由版本化组件、Skill 开关、TODO 状态和对应代码证据生成，在线状态分别读取写作、审核、材料润色和运维四个 Bot 的心跳；执行方式直接读取写作和审核任务类型的真实队列注册表，并显示“持久队列”或“实时执行”，不靠人工重复维护。不调用模型自由判断进度。控制台展示全部已知心跳，但不替代运维 Bot 自身的告警服务配置。架构节点关系由代码内明确关系表维护，关系端点必须经过自动化测试确认存在；仓库级测试要求每个已安装 Skill 至少映射到一个状态清单组件。信息流图使用固定分区和可暂停的原生 SVG 动效，只为七条主干关系绘线；垂直主链使用直线，写作和审核分支使用圆角转弯线。系统减少动态效果时自动禁用动画。它不依赖第三方关系图库、CDN 或 React 构建链，节点详情和状态清单仍由服务端生成。控制台只监听 `127.0.0.1`，并作为 macOS LaunchAgent 后台运行，不打开终端窗口。Git 查询是代码内固定的只读命令，不接受页面输入、不自动联网；任务和知识库在总览中只统计数量，不读取用户正文。项目状态仍以代码、核心文档和 Git 为唯一事实来源，控制台不维护第二套状态文件。
...
持久任务生效范围必须按具体 handler 判断：审核八类能力、直报、writer1、shenyinxie_news 和 internal_weekly 已接入专用队列；其他能力不能仅因公共执行器存在就宣称支持进程恢复和发送幂等。
