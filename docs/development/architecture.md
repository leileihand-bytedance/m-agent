...
5. PPT 专项审核第一阶段已接入，但只覆盖单份 PPTX 的可编辑内容和内部一致性。语言审核批次由 PPT 业务包自主切分；当模型明确输出超限时，按 PPT 对象边界拆半续审并合并结果。名称不一致另有双边原文和同名归一校验，均不依赖通用或 Word 审核引擎。先完成真实样本验收，再评估视觉版式、图片 OCR 和跨 PPT 审核。能读取或渲染 PPT 仍不等于已经能审核 PPT 版式。
