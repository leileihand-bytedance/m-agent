...
- `read_document_file`：统一读取当前任务目录内的 `.docx`、`.pdf`、`.pptx`，经过格式、路径、宏和异常压缩包校验后，返回标准材料片段并把完整解析结果保存到任务 `work/`。宏校验只针对主文档宏格式和真实 VBA 项目；PPT 图表内嵌的惰性 `.xlsb` 数据不会因内容类型名称含 `macroEnabled` 被误拒，且不会被执行或作为审核正文读取。旧 Word/PDF 工具保留兼容。
