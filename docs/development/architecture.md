...
- 统一企业微信入口适配：把更多 skill 接入同一个入口。
- 底座级文件消息处理：把写作 Bot 已验证的文件下载和任务组装能力下沉到公共入口，并补齐格式、大小、权限和跨网络素材入口方案。

入口层只负责接收消息、下载文件、返回结果，不写具体业务规则。
...
- 运维 Bot 轮询 `ops_events`，把未通知过的异常和提醒发送给管理员。
...
其中 `writer1` / `writer2` 已启用，已补齐 workflow、schema 和业务测试，并已接入 `bank_materials` 微众银行材料包和共享 `policy_research` 政策判断层；兼容层仍保留 `policy_materials` 作为低层政策材料包工具。当前已新增简报共享质量层：写作前会注入隐藏规划，多素材会先判断是否弱关联，生成后会做确定性规则检查和语义审查；后续仍需继续通过经授权样本收敛成稿质量和多素材整合表达。
...
1. 优化 `direct_report` 写作质量。
2. 优化 `writer1` / `writer2` 的成稿质量和多素材整合规则。
3. 把写作 Bot 的文件下载和短任务组装能力下沉为底座公共能力。
4. 把更多 skill 接入当前企业微信入口。
5. 包装 `app/review/` 为 `review` skill。
6. 在“上一稿改稿 v1”基础上扩展复杂多轮任务和人工确认。
