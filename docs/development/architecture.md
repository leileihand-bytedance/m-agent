...
- `app/platform/task_relations.py`：跨 Skill 任务关系层，按入口和 `userid` 维护多任务卡片、版本、材料角色、父子任务和待确认状态；固定区分续改、补料、派生、新建、回答追问、切换、取消和需要确认。目标选择依次尊重任务序号、明确标题或文种、用户当前稿件；“再改、继续改、接着改、这个直报/简报”等承接表达直接落到当前稿，不交给模型按主题猜测。剩余模糊场景才调用结构化模型，高置信度结果仍须通过目标任务和状态硬校验。
...
- `app/writing/task_execution.py`：直报和简报的持久任务适配层；简报只使用 `writer1` 一个队列 ID，入队前创建正式写作 job 和文件快照，后台按“处理、会话收尾、主动发送”三个检查点恢复。
...
直报和 `writer1` 的普通续改、补充/替换/参考材料已经进入写作持久队列。目标稿本身仍在生成时先保存要求并提示等待；不同稿件可继续排队，同一用户执行并发仍为 1，不同用户由队列并发上限共同调度。沿用旧任务结构另写一份会建立父子任务关系，只传递结构摘要，不自动混入旧稿事实。
