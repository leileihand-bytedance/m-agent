...
- 当前验收批次包含五类单项审核、直报、`writer1`、`writer2`：企业微信回调快速登记任务并返回实际业务类型的已受理提示，后台处理和交付。完整任务编号只在后台用于幂等、恢复、状态关联和排障；正常受理与重复提交不向用户展示，异常需要人工介入时才以“处理编号”显示。审核与写作使用不同 SQLite，避免 worker 互相领取不支持的任务。
