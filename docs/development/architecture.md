...
- `app/writing/intake.py`：写作 Bot 的短任务组装层，负责把用户分多条发送的意图、链接、文字和文件组装成一次结构化写作请求；当前可进入直报、单/多素材简报、综合调研整合、深银协动态、内参周报或文字润色。内参周报和深银协动态无需用户上传材料。待组装状态和文件持久化在 `M-Agent-Files/runtime/intake/`，有效期内可在 Bot 重启后恢复；结构化任务返回待澄清时不会提前清理，用户回答后沿原文件自动续跑。
...
- `app/review/rules/`：活动规则ID和规则族的权威索引，以及各审核类型的静态profile。纯文字、通用Word、静态HTML、内参和半月报已经显式接入各自profile；profile只选择规则，不按规则增加模型调用。内参和半月报复用共享模型运行、逐字证据、去重和阶段指标，但专属提示词与结构判断仍在原模块；PPT继续保留页码和对象证据及独立提示词，等待后续有限迁移。
...
- 七类单项审核使用审核专用队列，直报、writer1 和 writer2 使用写作专用队列。
- research_synthesis、shenyinxie_news、internal_weekly 和多文件联合审核尚未获得同等持久恢复能力。
...
- `read_web_page`：只读取公网 http/https 网页；拒绝 `file://`、localhost、内网 IP、云元数据地址和 DNS 解析到私网。请求关闭自动跳转，每一跳先校验目标再访问；已校验的公网 DNS 结果会固定到本次请求，并限制跳转次数和响应体大小。发布日期除常见新闻元数据外，也识别研究报告页面常用的 `citation_publication_date` 和 `DC.date`，供 Skill 做日期硬校验。
...
能力地图把建设成熟度和运行在线状态分开：成熟度由版本化的架构节点、Skill 开关、TODO 状态和对应代码证据生成，在线状态分别读取写作、审核、材料润色和运维四个 Bot 的心跳；不调用模型自由判断进度。控制台展示全部已知心跳，但不替代运维 Bot 自身的告警服务配置。节点关系同样由代码内明确关系表维护，关系端点必须经过自动化测试确认存在；仓库级测试还要求每个已安装 Skill 至少映射到一个架构能力节点。当前业务层显式展示直报、简报、综合调研、材料润色、深银协动态、内参周报、通用审核、静态 HTML、公文格式、多文件联合审核、单份 PPTX 低级错误审核，以及 `TODO-031` 规划中的审核共享核心。交互画布使用仓库内固定版本的 `vis-network 10.1.0`，不依赖 CDN 或 React 构建链；节点详情和状态清单仍由服务端生成。控制台只监听 `127.0.0.1`。Git 查询是代码内固定的只读命令，不接受页面输入、不自动联网；任务和知识库在总览中只统计数量，不读取用户正文。项目状态仍以代码、核心文档和 Git 为唯一事实来源，控制台不维护第二套状态文件。
...
持久任务生效范围必须按具体 handler 判断：七类单项审核、直报、writer1 和 writer2 已接入专用队列；其他能力不能仅因公共执行器存在就宣称支持进程恢复和发送幂等。
