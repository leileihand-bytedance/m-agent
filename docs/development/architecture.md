...
- `app/review/main.py`：审核 Bot 独立入口；普通文件按原有内容审核分流，显式格式审核和多文件联合审核通过审核模块自己的持久化组装层处理。确认属于单项审核后提交审核专用后台队列；多文件联合审核保留原执行方式。
...
- 后台任务执行器已完成生产竞态加固。纯文字、单个通用 Word、内参、半月报和公文格式接入审核专用持久队列；直报、`writer1`、`writer2` 接入独立写作队列。`research_synthesis` 和多文件联合审核仍未切流，必须按具体 handler 区分生效范围。

当前交付边界必须分成两类理解：

- **已经接入现有入口**：公共任务暂存、跨消息材料组装、DOCX/PDF/PPTX 安全读取、扫描 PDF 按需 OCR、结果附件重试和运维告警。这些能力已经直接改善当前写作或审核流程。
- **已经进入切流验收**：六类单项审核使用审核专用 SQLite；直报和两类简报使用写作专用 SQLite。九类任务都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。
...
- 当前验收批次包含六类单项审核、直报、`writer1`、`writer2`：企业微信回调快速登记任务并返回已受理提示，后台处理和交付。审核文件在下载前已经即时确认，单项入队后不再追加队列或类型提示；文件前的短格式要求先由持久 intake 识别和保存，再与随后 Word 文件组装为公文格式审核。PPTX 直接进入独立任务，较长结果通过同一持久检查点分段发送。审核队列的主动文字结果统一使用企业微信 SDK 支持的 Markdown 消息。完整任务编号只在后台用于幂等、恢复、状态关联和排障；正常受理与重复提交不向用户展示，异常需要人工介入时才以“处理编号”显示。审核与写作使用不同 SQLite，避免 worker 互相领取不支持的任务。
- 六类单项审核、直报、`writer1`、`writer2` 已分别接入审核/写作专用队列；下一步做真实重复消息、排队、重启和发送验收，再单独评估 `research_synthesis`。可安全重跑的阶段才允许恢复，不可确认的外部发送不自动重试。
...
审核 Bot 同样具备模块内持久化组装 v1：Word 格式审核可先发文件后补充指令；Word 内容审核默认不要求任何前后指令，文件到达后立即确认，再通过 `M_AGENT_REVIEW_AUTO_BATCH_SECONDS` 控制的后台静默窗口（默认 8 秒）自动区分单文件和 2 至 5 份联合审核。主文件综合文件名和正文引用识别，不按上传顺序决定；证据不足时才请求用户确认。待组装状态使用 `M_AGENT_REVIEW_INTAKE_TTL`，默认 1800 秒。单份 PPTX 不进入这套 Word 组装状态，下载和大小校验后直接提交独立 `review_pptx` 持久任务；它不参与公文格式或多文件联合审核。该实现通过公共任务动作、材料引用和提交模型与写作入口对齐；格式审核、主文件识别、静默窗口和 PPT 独立分流仍是审核自己的业务规则。

尚未完成：

- 六类单项审核的真实运行验收，以及综合调研、多文件联合审核向持久化执行器的后续分批切流；不能在未声明 handler 可恢复性时直接启用。
...
- 后台任务执行内核已支持幂等键、SQLite 持久化、全局/单用户/成本并发、租约 token、心跳、跨进程执行锁、周期恢复、取消、待补充后恢复、schema 迁移和状态版本防乱序；纯文字、四类单项 Word 和单份 PPTX 共六类单项审核已接入，多文件联合审核仍需单独设计多输入、多结果检查点后再切流。
...
4. 公共组装协议、持久化执行器和附件交付内核已完成；先验收已切流的六类单项审核、直报和简报，再单独评估综合调研与多文件联合审核的多结果恢复方案。
