...
- `app/review/html_parser.py`：只提取上传 HTML 的静态可见标题、段落、列表和表格行；忽略脚本、样式、注释、显式隐藏元素、关闭的 `dialog` 和折叠 `details` 的非摘要正文，只从真实 `meta` 读取编码声明；不执行 JavaScript、不联网加载资源，也不解释复杂 CSS 选择器。
