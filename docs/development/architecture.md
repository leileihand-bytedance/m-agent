...
- `app/writing/task_execution.py`：直报和简报的持久任务适配层；简报内部继续接受 `writer1`、`writer2` 两个队列 ID，入队前创建正式写作 job 和文件快照，后台按“处理、会话收尾、主动发送”三个检查点恢复。
...
- **已经进入切流验收**：审核八类能力使用审核专用 SQLite；直报、简报的两个兼容队列 ID 和深银协动态使用写作专用 SQLite。十二类 handler 都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态按统一三态保存，无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。
...
直报、`writer1` 和 `writer2` 的普通续改、补充/替换/参考材料已经进入写作持久队列。目标稿本身仍在生成时先保存要求并提示等待；不同稿件可继续排队，同一用户执行并发仍为 1，不同用户由队列并发上限共同调度。沿用旧任务结构另写一份会建立父子任务关系，只传递结构摘要，不自动混入旧稿事实。
...
当前登记的业务能力包括直报、统一简报、材料润色、综合调研整合、深银协动态和内参周报。简报以 `skills/writer1/` 作为唯一规则、提示词和 workflow 来源，`writer2` 只保留历史任务兼容配置与薄转发层；其他 Skill 各自维护业务规则、结构化模型和 workflow。所有 Skill 都只能调用底座授权工具。内参周报内容生产与 `app/review/` 的内参审核、半月报审核相互独立；分类规则复制到 Skill 自有目录后维护，运行时不依赖审核模块。
