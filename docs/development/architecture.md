...
直报和 `writer1` 的普通续改、补充/替换/参考材料已经进入写作持久队列。对当前直报或简报只要求“导出 Word”时，任务关系层按继续当前稿执行，Skill 直接复用上一稿内容，不调用写作模型；直报期号、总期号和落款日期等明确版头要求也按同一方式续接。目标稿本身仍在生成时先保存要求并提示等待；不同稿件可继续排队，同一用户执行并发仍为 1，不同用户由队列并发上限共同调度。沿用旧任务结构另写一份会建立父子任务关系，只传递结构摘要，不自动混入旧稿事实。
