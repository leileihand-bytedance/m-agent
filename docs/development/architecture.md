...
- `app/writing/intake.py`：写作 Bot 的短任务组装层，负责把用户分多条发送的意图、链接、文字和文件组装成一次结构化写作请求；当前可进入直报、单/多素材简报、综合调研整合或文字润色。待组装状态和文件持久化在 `M-Agent-Files/runtime/intake/`，有效期内可在 Bot 重启后恢复。
...
- 底座级多消息任务组装：继续把写作 Bot 和审核 Bot 已分别验证的持久化能力下沉到公共入口，统一处理暂存、超时、连续文件自动归集、可选显式开始、取消、格式、大小和任务隔离；正文与附件一致性仍留在审核业务层。
...
综合调研整合给模型材料增加 `material_role`：`outline` 表示唯一提纲，`source` 表示部门素材。Pydantic AI prompt 会显式展示该角色；提纲使用更大的均衡取样预算，避免普通上传材料的较短上下文预算截断提纲尾部。该标记只影响模型材料编排，不放宽文件访问范围。
...
- 待组装状态和文件采用原子写入，按入口和用户隔离；Bot 重启后会恢复有效期内的会话，过期或已提交的暂存文件会清理。
...
`research_synthesis` 与 `writer2` 分开：`writer2` 从多素材中自行提炼简报主题和结构；`research_synthesis` 把用户现成提纲作为不可随意改变的结构骨架，将各部门材料归入对应章节。第一版只做忠实整合、去重、必要衔接以及缺口/冲突标记，不联网补事实，不包含宣传化润色；超长材料逐条穷尽、第二阶段润色和 Word 导出由 `TODO-026` 后续推进。
...
用户先说“帮我做综合调研整合”
  ...
  -> 用户回复“开始写”
  -> research_synthesis 唯一识别提纲并按提纲归集
```
...
审核 Bot 同样具备模块内持久化组装 v1：格式审核可先发文件后补充指令；内容审核默认不要求任何前后指令，文件到达后立即确认，再通过 `M_AGENT_REVIEW_AUTO_BATCH_SECONDS` 控制的后台静默窗口（默认 8 秒）自动区分单文件和 2 至 5 份联合审核。主文件综合文件名和正文引用识别，不按上传顺序决定；证据不足时才请求用户确认。待组装状态使用 `M_AGENT_REVIEW_INTAKE_TTL`，默认 1800 秒。该实现保证审核功能先可用，但尚未与写作入口抽成同一底座接口。

尚未完成：

- 跨写作、审核共用的底座级多消息/多文件任务组装接口；两个入口已有各自的持久化 v1，但公共抽象尚未完成。
- 扫描 PDF 按需 OCR、PDF/PPT 页面渲染、复杂文档后台任务和公共附件交付。
...
- 写作入口支持综合调研多文件组装；文件名或用户说明必须能唯一指出提纲，否则 skill 追问而不按上传顺序猜测。
- 写作入口每次任务最多接收 10 份文件，文件总大小仍不超过 20MB；数量和总大小分别在任务组装阶段校验。
- 审核入口支持格式审核前后置触发，以及 `.docx` 单/多文件自动分流、可选显式开始、主文件确认、持久化恢复和任务隔离。
- 统一文档服务 v1 支持 DOCX/PDF/PPTX 安全解析、标准结构、页码或幻灯片定位和完整结果落盘；扫描 PDF 会标记为待 OCR。
...
2. 统一文档服务 v1 已完成；继续补扫描 PDF 按需 OCR、页面渲染和真实复杂文件回归，HTML 文件支持暂缓。
3. 用真实“正文 + 多附件”材料验收审核 Bot 已实现的 `.docx` 联合审核 v1，收敛误报、漏报、定位、耗时和回传问题。
4. 把写作 Bot 与审核 Bot 的持久化短任务组装下沉为底座公共能力，再扩展 PDF、Excel 和 PPT 联合审核。
5. 完成大文件回传后增加 PPT 专项审核；能读取 PPT 素材不等于已经能审核 PPT 版式。
