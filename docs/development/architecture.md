...
- `app/writing/task_execution.py`：直报、简报、综合调研、深银协动态和内参周报的持久任务适配层；简报只使用 `writer1` 一个队列 ID，入队前创建正式写作 job 和文件快照，后台按“处理、会话收尾、主动发送”三个检查点恢复。直报或简报生成正式 Word 时，标题正文文字和任务 `output/` 内 `.docx` 附件作为两个独立交付项发送和恢复；综合调研按完成提示和 `.docx` 初稿分别交付，内参周报的完成说明、Markdown 核对稿和 JSON 溯源清单作为三个独立交付项。每项主动发送结果还写入不含正文、文件名、路径和用户 ID 的状态日志。
...
- **已经进入切流验收**：审核八类能力使用审核专用 SQLite；直报、简报、综合调研、深银协动态和内参周报使用写作专用 SQLite。十三类 handler 都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态按统一三态保存，无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。综合调研的完成提示和 Word、内参周报的完成说明、Markdown 和 JSON 都分别记录检查点，任一已经确认送达的交付项不会因恢复重复发送。
...
当前登记的业务能力包括直报、统一简报、材料润色、综合调研整合、深银协动态和内参周报。简报以 `skills/writer1/` 作为唯一规则、提示词、workflow、权限和队列来源；历史持久化数据中的退役 ID 只在读取边界归一，不登记为 Skill。其他 Skill 各自维护业务规则、结构化模型和 workflow。所有 Skill 都只能调用底座授权工具。内参周报内容生产与 `app/review/` 的内参审核、半月报审核相互独立；分类规则复制到 Skill 自有目录后维护，运行时不依赖审核模块。
