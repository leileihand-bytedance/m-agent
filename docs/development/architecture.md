...
- 底座级多消息任务组装：继续把写作 Bot 已验证的持久化能力下沉到公共入口，统一处理暂存、超时、显式开始、取消、格式、大小和任务隔离，供写作与多文件审核复用。
...
- 跨写作、审核共用的底座级多消息/多文件任务组装接口；写作入口已有持久化 v1，审核尚未接入。
...
3. 把写作 Bot 的持久化短任务组装接口下沉为底座公共能力，并扩展为审核可复用的多文件任务组装。
4. 在公共组装层和统一文档服务之上实现“正文 + 多附件”联合审核。
