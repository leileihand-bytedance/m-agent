...
- `app/writing/task_execution.py`：直报和简报的持久任务适配层；简报只使用 `writer1` 一个队列 ID，入队前创建正式写作 job 和文件快照，后台按“处理、会话收尾、主动发送”三个检查点恢复。简报生成正式 Word 时，标题正文文字和任务 `output/` 内附件作为两个独立交付项发送和恢复。
...
直报和 `writer1` 的普通续改、补充/替换/参考材料已经进入写作持久队列。对当前简报只要求“导出 Word”也作为继续任务执行，但工作流直接复用上一稿内容，不调用写作模型。目标稿本身仍在生成时先保存要求并提示等待；不同稿件可继续排队，同一用户执行并发仍为 1，不同用户由队列并发上限共同调度。沿用旧任务结构另写一份会建立父子任务关系，只传递结构摘要，不自动混入旧稿事实。
