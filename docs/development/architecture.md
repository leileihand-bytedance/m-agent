...
- `app/writing/intake.py`：写作 Bot 的短任务组装层，负责把用户分多条发送的意图、链接、文字和文件组装成一次结构化写作请求；当前可进入直报、单/多素材简报、综合调研整合、深银协动态或文字润色。待组装状态和文件持久化在 `M-Agent-Files/runtime/intake/`，有效期内可在 Bot 重启后恢复；结构化任务返回待澄清时不会提前清理，用户回答后沿原文件自动续跑。
- `app/review/main.py`：审核 Bot 独立入口；Word 按原有内容审核分流，单个 `.html/.htm` 直接进入静态文字审核，显式格式审核和多文件联合审核通过审核模块自己的持久化组装层处理。确认属于单项审核后提交审核专用后台队列；多文件联合审核保留原执行方式。
...
- 后台任务执行器已完成生产竞态加固。纯文字、单个通用 Word、内参、半月报、公文格式、单个静态 HTML 和单份 PPTX 接入审核专用持久队列；直报、`writer1`、`writer2` 接入独立写作队列。`research_synthesis` 和多文件联合审核仍未切流，必须按具体 handler 区分生效范围。

当前交付边界必须分成两类理解：

- **已经接入现有入口**：公共任务暂存、跨消息材料组装、DOCX/PDF/PPTX 安全读取、扫描 PDF 按需 OCR、结果附件重试和运维告警。这些能力已经直接改善当前写作或审核流程。
- **已经进入切流验收**：七类单项审核使用审核专用 SQLite；直报和两类简报使用写作专用 SQLite。十类任务都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。
- **底座已完成、其他业务待切流**：持久化任务排队、重复消息幂等、分级并发、取消和进程重启恢复。`research_synthesis` 和多文件联合审核尚未获得这套生产行为，不能根据公共内核存在就标为已接入。

因此，本轮 P1 的实际作用是把“收材料、跑任务、读文件、发结果”四类公共基础设施补齐。下一阶段是生产接入和故障验收，不是再增加一套任务框架。

后续接入：

- 当前验收批次包含七类单项审核、直报、`writer1`、`writer2`：企业微信回调快速登记任务并返回已受理提示，后台处理和交付。审核文件在下载前已经即时确认，单项入队后不再追加队列或类型提示；文件前的短格式要求先由持久 intake 识别和保存，再与随后 Word 组装为公文格式审核。HTML 和 PPTX 都不进入 Word 多文件归集，分别直接进入独立任务；HTML 只发送 Markdown 审核消息，PPTX 较长结果通过同一持久检查点分段发送。审核队列的主动文字结果统一使用企业微信 SDK 支持的 Markdown 消息。完整任务编号只在后台用于幂等、恢复、状态关联和排障；正常受理与重复提交不向用户展示，异常需要人工介入时才以“处理编号”显示。审核与写作使用不同 SQLite，避免 worker 互相领取不支持的任务。
- 七类单项审核、直报、`writer1`、`writer2` 已分别接入审核/写作专用队列；下一步做真实重复消息、排队、重启和发送验收，再单独评估 `research_synthesis`。可安全重跑的阶段才允许恢复，不可确认的外部发送不自动重试。
- 审核 Bot 继续保持独立入口；多文件联合审核仍使用现有路径，不因单项审核切流自动切换。
...
其中 `writer1` / `writer2` 已启用，已补齐 workflow、schema 和业务测试，并已接入 `bank_materials` 微众银行材料包和共享 `policy_research` 政策判断层；兼容层仍保留 `policy_materials` 作为低层政策材料包工具。`rewrite` 也已允许调用 `bank_materials`，但用途更窄：workflow 先用明确的微众机构、产品或“我行 + 微众业务语境”信号做关联闸门，只有命中时才检索，所得语料仅用于核对机构名、产品名和已有标准表述，不得补入原文没有的数据、日期、荣誉或新事实。当前简报真实口径已明确为面向深圳市金融办、南山区、前海管理局、深圳人行、深圳金监局等地方政府和监管部门，重点展示微众银行近期动态及成果，篇幅正常控制在 `1000` 字左右、最长不超过 `1200` 字。当前已新增简报共享质量层：写作前会注入隐藏规划，并按经授权样本识别综合成果型、机制成果型、产品工具型、平台合作型、标准引领型、能力建设型、外部认可型、活动亮相型、专项治理型等类型；多素材会先判断是否弱关联，生成后会做确定性规则检查和语义审查；后续仍需继续通过经授权样本收敛成稿质量和多素材整合表达。
...
`shenyinxie_news` 同样不依赖用户提前提供素材，但时间范围必须由用户明确指定月份和上半月/下半月；未明确时工作流在调用任何搜索工具前返回待澄清，写作入口保留原任务，用户答复后仍恢复为 `shenyinxie_news`。明确范围后，通过 ToolGateway 调用 DeepSeek 原生 Web Search。检索采用两层白名单：先查中央/监管/全国财经/深圳党媒，完整专题稿不足 3 篇时才查已核验的全国金融行业媒体和广东主流媒体；所有查询携带准确起止日期并补充科技创新、数字普惠金融、党建等成果主题，最终仍以网页发布日期为准。搜索结果只形成候选池，代码先执行日期、域名、正文可读性等硬门槛，再通过 `llm_writer` 的结构化输出判断正面成果、核心主体和“全文/摘编/拒绝”。完整专题稿优先；综合稿只有存在可独立成立的微众银行成果段落时才允许摘编，且每段必须按原顺序逐字存在于原文。Word 生成直接复制由用户确认案例净化得到的 Skill 自有母版，保留其 A4 页面、页边距和段落角色样式，仅原位替换标题、时间、动态标题、逐段正文与来源行；不在运行时读取桌面案例，也不重新构造生产版式。正文按自然段拆成真实 Word 段落，链接写为外部可点击关系，未使用动态块完整删除；摘编稿另展示原报道标题和摘编说明。0 篇合格报道时返回明确提示，不生成空白 Word。

网页发布日期原则上来自原文结构化字段、时间标签或正文明确日期。人民日报旧版页面的 `publishdate` 可能固定为错误旧值，因此仅对 `paper.people.com.cn` 启用受限校验：请求正文路径中的报纸期号日期必须同时出现在页面内期号目录或正文 URL 标记中，才作为发布日期；其他站点不得用 URL 数字替代发布日期。同稿去重会先移除媒体站点追加的短标题尾缀，再比较标题和正文，避免同一篇稿件以不同站点标题重复入选。
...
能力地图把建设成熟度和运行在线状态分开：成熟度由版本化的架构节点、Skill 开关、TODO 状态和对应代码证据生成，在线状态分别读取写作、审核、材料润色和运维四个 Bot 的心跳；不调用模型自由判断进度。控制台展示全部已知心跳，但不替代运维 Bot 自身的告警服务配置。节点关系同样由代码内明确关系表维护，关系端点必须经过自动化测试确认存在；仓库级测试还要求每个已安装 Skill 至少映射到一个架构能力节点。当前业务层显式展示直报、简报、综合调研、材料润色、深银协动态、通用审核、静态 HTML、公文格式、多文件联合审核、单份 PPTX 低级错误审核，以及 `TODO-031` 规划中的审核共享核心。交互画布使用仓库内固定版本的 `vis-network 10.1.0`，不依赖 CDN 或 React 构建链；节点详情和状态清单仍由服务端生成。控制台只监听 `127.0.0.1`。Git 查询是代码内固定的只读命令，不接受页面输入、不自动联网；任务和知识库在总览中只统计数量，不读取用户正文。项目状态仍以代码、核心文档和 Git 为唯一事实来源，控制台不维护第二套状态文件。
...
当前 `gateway/wecom.py` 是纯 Python 核心，已经有单元测试。写作 Bot 的真实企业微信入口目前由 `app/writing/bot.py` 适配，并已经调用 `PlatformApp`。独立材料润色入口由 `app/rewrite_bot/bot.py` 适配；该入口在创建 `PlatformApp` 时把 Skill 注册表限制为 `rewrite`，因此不是仅靠提示词约束业务范围。
...
企业微信文字消息
  ...
  -> SkillRegistry(include_skill_ids={"rewrite"})
  -> PlatformApp / AccessPolicy
  -> skills/rewrite/
  -> 企业微信文字回复
```

该入口不接收文件或网页链接，任务保存在 `tasks/writing/rewrite/`，会话保存在 `runtime/conversations/rewrite-bot/`。它与原写作 Bot 共用模型配置、权限策略、用户名称表、日志和受限工具层，但不共用活跃稿件上下文。用户只粘贴原文时，`app/rewrite_bot/intake.py` 先把原文保存在 `runtime/intake/rewrite-bot/` 并追问修改方向；用户补充要求后再把“原文 + 要求”结构化提交给 `rewrite`。同条消息已经包含原文和要求、或是在继续修改本 Bot 上一版结果时直接处理。明确要求写直报、写简报、综合调研、审核或感谢信时，入口会说明这里只提供材料润色，不把请求交给其他 Skill。
...
审核 Bot 同样具备模块内持久化组装 v1：Word 格式审核可先发文件后补充指令；Word 内容审核默认不要求任何前后指令，文件到达后立即确认，再通过 `M_AGENT_REVIEW_AUTO_BATCH_SECONDS` 控制的后台静默窗口（默认 8 秒）自动区分单文件和 2 至 5 份联合审核。主文件综合文件名和正文引用识别，不按上传顺序决定；证据不足时才请求用户确认。待组装状态使用 `M_AGENT_REVIEW_INTAKE_TTL`，默认 1800 秒。单个静态 HTML 和单份 PPTX 不进入这套 Word 组装状态，下载和大小校验后分别提交独立持久任务；它们不参与公文格式或多文件联合审核。该实现通过公共任务动作、材料引用和提交模型与写作入口对齐；格式审核、主文件识别、静默窗口、HTML 和 PPT 独立分流仍是审核自己的业务规则。

尚未完成：

- 七类单项审核的真实运行验收，以及综合调研、多文件联合审核向持久化执行器的后续分批切流；不能在未声明 handler 可恢复性时直接启用。
...
- 审核入口支持单个 `.html/.htm` 静态可见文字审核：不执行脚本或联网资源，只返回 Markdown 审核消息。
- 审核入口支持单份 `.pptx` 低级错误审核：独立业务包只读取可编辑文本框、表格和可读图表内容，排除图片文字和备注；确定性规则、模型候选和双边原文证据校验均不依赖其他业务审核引擎。名称不一致候选必须提供同一专有名称的两处不同原文，代码在忽略大小写、宽窄字符和名称内常见分隔符后再次确认两边属于同一名称；单边猜测、不同名称、相同脚注、脚注/列表标记开头和常见注释语句不会形成名称问题，反向重复候选只保留一条。跨页候选由代码再次核对明确年份、时间范围、单位和目标/实际状态；用户可见的问题说明只由代码按类别生成，不透传模型自由描述，从结构上排除修改建议。每个语言批次和全篇一致性阶段按输入 SHA 写入原子断点，重启只执行缺失阶段。结果通过持久任务以纯文字分段发送。
- 统一文档服务 v1 支持 DOCX/PDF/PPTX 安全解析、标准结构、页码或幻灯片定位和完整结果落盘；PPTX 文本保留段落层级缩进供同级序号分组，图表系列底层数值读取失败时记录带幻灯片位置的结构化告警。DOCX 内嵌图片会提取到任务 `work/` 并在正文原位置生成未读取提醒，长材料均衡抽样强制保留这些提醒；扫描 PDF 会标记为待 OCR。
...
- 公共附件交付已接入写作和审核 Bot：默认支持企业微信 SDK 的 100 个 512KB 分片上限，统一动态超时、重试和安全运维事件；超限或重试失败时返回“处理编号”供管理员人工取回。当前不自动压缩用户图片，因此不会静默改变文档画质。
- 后台任务执行内核已支持幂等键、SQLite 持久化、全局/单用户/成本并发、租约 token、心跳、跨进程执行锁、周期恢复、取消、待补充后恢复、schema 迁移和状态版本防乱序；七类单项审核已接入，多文件联合审核仍需单独设计多输入、多结果检查点后再切流。
...
2. 统一文档服务 OCR 和页面渲染已完成；继续用真实复杂文件扩充回归。写作材料 HTML、动态渲染和 HTML 视觉版式仍暂缓；独立审核 Bot 已支持单个静态 HTML 的文字与内部数据一致性审核。
3. 用真实“正文 + 多附件”材料验收审核 Bot 已实现的 `.docx` 联合审核 v1，收敛误报、漏报、定位、耗时和回传问题。
4. 公共组装协议、持久化执行器和附件交付内核已完成；先验收已切流的七类单项审核、直报和简报，再单独评估综合调研与多文件联合审核的多结果恢复方案。
5. PPT 专项审核第一阶段已接入，但只覆盖单份 PPTX 的可编辑内容和内部一致性。语言审核批次由 PPT 业务包自主切分；当模型明确输出超限时，按 PPT 对象边界拆半续审并合并结果。名称不一致另有双边原文和同名归一校验，均不依赖通用或 Word 审核引擎。首份 20 页真实 PPT 已完成端到端实测并修复输出截断和名称误报；继续扩大真实样本验收后，再评估视觉版式、图片 OCR 和跨 PPT 审核。能读取或渲染 PPT 仍不等于已经能审核 PPT 版式。
