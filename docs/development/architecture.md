...
- `app/review/rules/`：活动规则ID和规则族的权威索引，以及各审核类型的静态profile。纯文字、通用Word和静态HTML已经显式接入各自profile；profile只选择规则，不按规则增加模型调用。内参、半月报和PPT仍按分批计划迁移，PPT保留页码和对象证据及独立提示词。
...
- research_synthesis、shenyinxie_news 和多文件联合审核尚未获得同等持久恢复能力。
...
当前登记的业务能力包括直报、单素材简报、多素材简报、材料润色、综合调研整合和深银协动态。每个 Skill 自己维护业务规则、提示词、结构化模型和 workflow，但只能调用底座授权工具。
...
- `app/writing/`：直报、简报、综合调研、深银协动态，并可路由文字润色。
