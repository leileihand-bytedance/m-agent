...
1. 架构图说明系统如何工作，并展示全部已登记架构节点：业务主链、底座的接入/编排/运行三层、写作和审核子能力、受限工具、知识资产及治理控制面。它只为七条主干关系绘制动态线，其他依赖通过固定分区和层级表达；写作与审核分别使用绿色和蓝色。身份隔离、工具白名单、结构化输出、幂等恢复和交付确认作为贯穿任务全程的五道约束单独呈现。
...
状态清单把建设成熟度、运行在线状态和执行方式分开：成熟度由版本化组件、Skill 开关、TODO 状态和对应代码证据生成，在线状态分别读取写作、审核、材料润色和运维四个 Bot 的心跳；执行方式直接读取写作和审核任务类型的真实队列注册表，并显示“持久队列”或“实时执行”，不靠人工重复维护。不调用模型自由判断进度。控制台展示全部已知心跳，但不替代运维 Bot 自身的告警服务配置。架构节点关系由代码内明确关系表维护，关系端点必须经过自动化测试确认存在；仓库级测试要求每个已安装 Skill 至少映射到一个状态清单组件。信息流图使用固定分区和可暂停的原生 SVG 动效，只为七条主干关系绘线；直接路径没有节点阻挡时使用直线箭头，必须避让时才使用圆角直角路径。系统减少动态效果时自动禁用动画。它不依赖第三方关系图库、CDN 或 React 构建链，节点详情和状态清单仍由服务端生成。控制台只监听 `127.0.0.1`。Git 查询是代码内固定的只读命令，不接受页面输入、不自动联网；任务和知识库在总览中只统计数量，不读取用户正文。项目状态仍以代码、核心文档和 Git 为唯一事实来源，控制台不维护第二套状态文件。
