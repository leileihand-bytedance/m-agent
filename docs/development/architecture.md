...
- PDF/PPT 等格式的解析 JSON、逐页或逐张渲染图片、OCR 结果和其他中间产物统一保存在任务 `work/`；需要返回给用户的报告、标注或生成文件保存在 `output/`。
...
4. 建立统一文档服务，先补齐 PDF 按页解析、长文分块、扫描页判断和标准材料结构，为多文件与 PPT 共用；HTML 文件支持暂缓。
5. 在公共组装层和统一文档服务之上实现“正文 + 多附件”联合审核。
6. 完成大文件回传后增加 PPT 审核。
7. 直报、简报和改稿保持样本驱动迭代；统一企业微信入口和 `review` skill 包装暂不作为近期前置条件。
8. 需要复杂长任务时再引入 LangGraph。
