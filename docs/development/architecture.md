...
- 后台任务执行器已完成生产竞态加固。单个 Word 通用审核接入审核专用持久队列；直报、`writer1`、`writer2` 接入独立写作队列。`research_synthesis`、内参、半月报、公文格式和多文件审核仍未切流，必须按具体 handler 区分生效范围。

当前交付边界必须分成两类理解：

- **已经接入现有入口**：公共任务暂存、跨消息材料组装、DOCX/PDF/PPTX 安全读取、扫描 PDF 按需 OCR、结果附件重试和运维告警。这些能力已经直接改善当前写作或审核流程。
- **已经进入切流验收**：单个 Word 通用审核使用审核专用 SQLite；直报和两类简报使用写作专用 SQLite。四类任务都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。
- **底座已完成、其他业务待切流**：持久化任务排队、重复消息幂等、分级并发、取消和进程重启恢复。`research_synthesis` 和其余审核任务尚未获得这套生产行为，不能根据公共内核存在就标为已接入。

因此，本轮 P1 的实际作用是把“收材料、跑任务、读文件、发结果”四类公共基础设施补齐。下一阶段是生产接入和故障验收，不是再增加一套任务框架。

后续接入：

- 当前验收批次包含单个 Word 通用审核、直报、`writer1`、`writer2`：企业微信回调快速登记任务并返回任务编号，后台处理和交付。审核与写作使用不同 SQLite，避免 worker 互相领取不支持的任务。
- 单个 Word 通用审核、直报、`writer1`、`writer2` 已分别接入审核/写作专用队列；下一步做真实重复消息、排队、重启和发送验收，再单独评估 `research_synthesis`。可安全重跑的阶段才允许恢复，不可确认的外部发送不自动重试。
- 审核 Bot 继续保持独立入口；内参、半月报、公文格式、多文件联合审核和文字审核仍使用现有路径，不因通用审核试点自动切换。
...
- 单个 Word 通用审核的真实运行验收，以及现有写作、其他审核重任务向持久化执行器的后续分批切流；不能在未声明 handler 可恢复性时直接全量启用。
...
- 后台任务执行内核已支持幂等键、SQLite 持久化、全局/单用户/成本并发、租约 token、心跳、跨进程执行锁、周期恢复、取消、待补充后恢复、schema 迁移和状态版本防乱序；单个 Word 通用审核已接入，其他具体任务仍需逐项切流验收。
...
4. 公共组装协议、持久化执行器和附件交付内核已完成；先验收已切流的单个 Word 通用审核，再按直报、简报和综合调研顺序观察幂等、恢复、耗时和告警。
