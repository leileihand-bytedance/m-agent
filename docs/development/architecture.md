...
- `app/writing/task_execution.py`：直报、简报、深银协动态和内参周报的持久任务适配层；简报只使用 `writer1` 一个队列 ID，入队前创建正式写作 job 和文件快照，后台按“处理、会话收尾、主动发送”三个检查点恢复。直报或简报生成正式 Word 时，标题正文文字和任务 `output/` 内 `.docx` 附件作为两个独立交付项发送和恢复；内参周报的完成说明、Markdown 核对稿和 JSON 溯源清单作为三个独立交付项。
...
- 后台任务执行器已完成生产竞态加固。纯文字、单个通用 Word、内参、半月报、公文格式、单个静态 HTML、单份 PPTX 和多文件联合审核接入审核专用持久队列；直报、`writer1`、`shenyinxie_news` 和 `internal_weekly` 接入独立写作队列。`research_synthesis` 仍为实时执行，必须按具体 handler 区分生效范围。
...
- **已经进入切流验收**：审核八类能力使用审核专用 SQLite；直报、简报、深银协动态和内参周报使用写作专用 SQLite。十二类 handler 都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态按统一三态保存，无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。内参周报的 Markdown 和 JSON 分别记录检查点，任一已经确认送达的文件不会因恢复重复发送。
- **底座已完成、其他业务待切流**：持久化任务排队、重复消息幂等、分级并发、取消和进程重启恢复。`research_synthesis` 尚未获得这套生产行为，不能根据公共内核存在就标为已接入。

当前入口接入边界：

- 审核八类能力使用审核专用队列，直报、writer1、shenyinxie_news 和 internal_weekly 使用写作专用队列。
- `research_synthesis` 尚未获得同等持久恢复能力；`rewrite` 属于即时多轮交互，不作为当前后台初稿队列任务。
