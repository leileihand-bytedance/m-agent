...
- `app/writing/intake.py`：写作 Bot 的短任务组装层，负责把用户分多条发送的意图、链接、文字和文件组装成一次结构化写作请求；待组装状态和文件持久化在 `M-Agent-Files/runtime/intake/`，有效期内可在 Bot 重启后恢复。
- `app/review/main.py`：审核 Bot 独立入口；普通文件按原有内容审核分流，显式格式审核和多文件联合审核通过审核模块自己的持久化组装层处理。
- `app/review/intake.py`：审核 Bot 过渡期短任务组装。格式审核支持“指令在前或文件在前”；联合审核支持 2 至 5 份 `.docx`、显式开始、取消、超时和主文件确认。系统不按上传顺序默认正文，状态按入口和用户隔离并可在 Bot 重启后恢复。
- `app/review/multi_file_reviewer.py`：审核业务层的多文件规则。逐份复用原有审核引擎，再检查附件缺失、重复、未引用、编号名称错配和跨文件逻辑；模型问题必须同时命中两份文件中的真实原文证据。

后续新增：

- 底座级多消息任务组装：继续把写作 Bot 和审核 Bot 已分别验证的持久化能力下沉到公共入口，统一处理暂存、超时、显式开始、取消、格式、大小和任务隔离；正文与附件一致性仍留在审核业务层。
...
- 用五层框架图展示用户入口、通用底座、业务功能、工具知识库、运维数据及各能力建设状态。
- 展示底座、写作、审核、知识库、入口运维和管理后台六个板块的当前情况、最新提交和首要待办。
- 保留 Skill 开关、用户权限和最近写作任务摘要管理。

能力地图把建设成熟度和运行在线状态分开：成熟度由版本化的架构节点、Skill 开关、TODO 状态和对应代码证据生成，在线状态只来自 Bot 心跳；不调用模型自由判断进度。控制台只监听 `127.0.0.1`。Git 查询是代码内固定的只读命令，不接受页面输入、不自动联网；任务和知识库在总览中只统计数量，不读取用户正文。项目状态仍以代码、核心文档和 Git 为唯一事实来源，控制台不维护第二套状态文件。
...
审核 Bot 同样具备模块内持久化组装 v1：格式审核可先发文件后补充指令；联合审核要求用户明确进入模式、连续发送文件并说“开始审核”。它使用 `M_AGENT_REVIEW_INTAKE_TTL`，默认也是 1800 秒。该实现保证审核功能先可用，但尚未与写作入口抽成同一底座接口。
...
- 审核入口支持格式审核前后置触发，以及 `.docx` 联合审核的显式开始、主文件确认、重启恢复和任务隔离。
