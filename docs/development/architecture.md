...
- 审核入口支持单份 `.pptx` 低级错误审核：独立业务包只读取可编辑文本框、表格和可读图表内容，排除图片文字和备注；确定性规则、模型候选和双边原文证据校验均不依赖其他业务审核引擎。跨页候选由代码再次核对明确年份、时间范围、单位和目标/实际状态，模型说明与最终格式化均清除修改建议；每个语言批次和全篇一致性阶段按输入 SHA 写入原子断点，重启只执行缺失阶段。结果通过持久任务以纯文字分段发送。
