...
当前登记的业务能力包括直报、统一简报、材料润色、综合调研整合、深银协动态和内参周报。简报以 `skills/writer1/` 作为唯一规则、提示词、workflow、权限和队列来源；历史持久化数据中的退役 ID 只在读取边界归一，不登记为 Skill。其他 Skill 各自维护业务规则、结构化模型和 workflow。所有 Skill 都只能调用底座授权工具。内参周报内容生产与 `app/review/` 的内参审核、半月报审核相互独立；分类规则复制到 Skill 自有目录后维护，运行时不依赖审核模块。该 Skill 先交付可追溯核对稿，只有用户明确批准同一 `draft_version` 后才复制净化母版生成 Word；批准后导出复用任务关系层保存的当前版本正文和受限文档元数据，不重新检索或调用模型。
