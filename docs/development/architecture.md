...
- 后台任务执行器已完成生产竞态加固，单个 Word 通用审核已作为首个 handler 接入审核专用持久队列。其他写作和审核长任务尚未切换；必须把“该分支可排队恢复”与“所有 Bot 都能自动续跑”严格区分。

当前交付边界必须分成两类理解：

- **已经接入现有入口**：公共任务暂存、跨消息材料组装、DOCX/PDF/PPTX 安全读取、扫描 PDF 按需 OCR、结果附件重试和运维告警。这些能力已经直接改善当前写作或审核流程。
- **已经进入第一批切流**：单个 Word 通用审核使用审核专用 SQLite 队列，支持快速受理、消息幂等、后台执行、检查点恢复和主动交付。队列最终结果只尝试发送一次，发送状态无法确认时宁可停止自动重发并告警，也不冒险重复发送；worker 意外退出会告警并自动重启。单文件原件仍按原规则保留 30 分钟，支持随后追加格式审核。
- **底座已完成、其他业务待切流**：持久化任务排队、重复消息幂等、分级并发、取消和进程重启恢复。内核与竞态测试已经完成，但只有具体写作或审核 handler 接入并验证后，对应业务才真正获得这些能力。

因此，本轮 P1 的实际作用是把“收材料、跑任务、读文件、发结果”四类公共基础设施补齐。下一阶段是生产接入和故障验收，不是再增加一套任务框架。

后续接入：

- 第一批只接单个 Word 通用审核：企业微信回调快速登记任务并返回任务编号，后台审核和交付，重点验证重复消息、进程重启、多用户排队和发送幂等。审核专用数据库位于 `runtime/task-execution/review.sqlite3`，避免其他执行器领取审核任务。
- 通用审核稳定后接入直报，再依次接入 `writer1`、`writer2`、`research_synthesis`；可安全重跑的任务才允许恢复，不可安全重跑的任务在中断后明确失败并通知运维。
