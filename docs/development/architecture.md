...
- 尚未统一两套状态机的材料、等待补充、开始、取消和提交协议，因此 `TODO-003` 仍是进行中，不能把“公共存储已完成”写成“公共组装已完成”。

后续新增：

- 底座级多消息任务组装：继续把写作 Bot 和审核 Bot 已分别验证的持久化能力下沉到公共入口，统一处理暂存、超时、连续文件自动归集、可选显式开始、取消、格式、大小和任务隔离；正文与附件一致性仍留在审核业务层。
- 后台任务执行与恢复：为长文审核、OCR、PPT 渲染和多阶段写作提供持久队列、消息幂等、并发限制、取消和重启恢复；第一版不要求引入 LangGraph。
...
- 跨写作、审核共用的底座级多消息/多文件业务组装协议；公共持久化、安全恢复和文件限制内核已完成，两套业务状态机尚未统一。
- 持久化后台任务执行器；当前长任务仍主要依赖消息回调、线程卸载或进程内异步任务，缺少幂等、并发限制和重启恢复。
...
4. 在公共暂存内核基础上统一写作和审核的任务组装协议，并建设持久化后台任务执行与恢复。
5. 完成公共附件回传后增加 PPT 专项审核；能读取 PPT 素材不等于已经能审核 PPT 版式。
