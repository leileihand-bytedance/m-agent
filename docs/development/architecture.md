...
- 后台任务执行器已完成生产竞态加固。纯文字、单个通用 Word、内参、半月报、公文格式和单个静态 HTML 接入审核专用持久队列；直报、`writer1`、`writer2` 接入独立写作队列。`research_synthesis` 和多文件联合审核仍未切流，必须按具体 handler 区分生效范围。
...
- 当前验收批次包含六类单项审核、直报、`writer1`、`writer2`：企业微信回调快速登记任务并返回已受理提示，后台处理和交付。审核文件在下载前已经即时确认，单项入队后不再追加队列或类型提示；文件前的短格式要求先由持久 intake 识别和保存，再与随后 Word 组装为公文格式审核。HTML 不进入 Word 多文件归集，独立解析静态可见文字并只发送 Markdown 审核消息。审核队列的主动文字结果统一使用企业微信 SDK 支持的 Markdown 消息。完整任务编号只在后台用于幂等、恢复、状态关联和排障；正常受理与重复提交不向用户展示，异常需要人工介入时才以“处理编号”显示。审核与写作使用不同 SQLite，避免 worker 互相领取不支持的任务。
...
- 六类单项审核（含静态 HTML）的真实运行验收，以及综合调研、多文件联合审核向持久化执行器的后续分批切流；不能在未声明 handler 可恢复性时直接启用。
...
- 后台任务执行内核已支持幂等键、SQLite 持久化、全局/单用户/成本并发、租约 token、心跳、跨进程执行锁、周期恢复、取消、待补充后恢复、schema 迁移和状态版本防乱序；六类单项审核已接入，多文件联合审核仍需单独设计多输入、多结果检查点后再切流。
