...
- **已经进入切流验收**：五类单项审核使用审核专用 SQLite；直报和两类简报使用写作专用 SQLite。八类任务都支持快速受理、消息幂等、后台执行、检查点恢复和主动交付；发送状态无法确认时停止自动重发并告警，worker 意外退出会告警并自动重启。
...
- 当前验收批次包含五类单项审核、直报、`writer1`、`writer2`：企业微信回调快速登记任务并返回已受理提示，后台处理和交付。审核文件在下载前已经即时确认，单项入队后不再追加队列或类型提示；文件前的短格式要求先由持久 intake 识别和保存，再与随后文件组装为公文格式审核。审核队列的主动文字结果统一使用企业微信 SDK 支持的 Markdown 消息。完整任务编号只在后台用于幂等、恢复、状态关联和排障；正常受理与重复提交不向用户展示，异常需要人工介入时才以“处理编号”显示。审核与写作使用不同 SQLite，避免 worker 互相领取不支持的任务。
- 五类单项审核、直报、`writer1`、`writer2` 已分别接入审核/写作专用队列；下一步做真实重复消息、排队、重启和发送验收，再单独评估 `research_synthesis`。可安全重跑的阶段才允许恢复，不可确认的外部发送不自动重试。
...
- 五类单项审核的真实运行验收，以及综合调研、多文件联合审核向持久化执行器的后续分批切流；不能在未声明 handler 可恢复性时直接启用。
...
- 后台任务执行内核已支持幂等键、SQLite 持久化、全局/单用户/成本并发、租约 token、心跳、跨进程执行锁、周期恢复、取消、待补充后恢复、schema 迁移和状态版本防乱序；五类单项审核已接入，多文件联合审核仍需单独设计多输入、多结果检查点后再切流。
...
4. 公共组装协议、持久化执行器和附件交付内核已完成；先验收已切流的五类单项审核、直报和简报，再单独评估综合调研与多文件联合审核的多结果恢复方案。
