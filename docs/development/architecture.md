...
该入口不接收文件或网页链接，任务保存在 `tasks/writing/rewrite/`，会话保存在 `runtime/conversations/rewrite-bot/`。它与原写作 Bot 共用模型配置、权限策略、用户名称表、日志和受限工具层，但不共用活跃稿件上下文。用户可直接粘贴原文而无需重复说明“润色”；明确要求写直报、写简报、综合调研、审核或感谢信时，入口会说明这里只提供材料润色，不把请求交给其他 Skill。
