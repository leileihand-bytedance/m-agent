...
- 审核入口支持单份 `.pptx` 低级错误审核：独立业务包只读取可编辑文本框、表格和可读图表内容，排除图片文字和备注；确定性规则、模型候选和双边原文证据校验均不依赖其他业务审核引擎。跨页候选由代码再次核对明确年份、时间范围、单位和目标/实际状态；用户可见的问题说明只由代码按类别生成，不透传模型自由描述，从结构上排除修改建议。每个语言批次和全篇一致性阶段按输入 SHA 写入原子断点，重启只执行缺失阶段。结果通过持久任务以纯文字分段发送。
...
5. PPT 专项审核第一阶段已接入，但只覆盖单份 PPTX 的可编辑内容和内部一致性。语言审核批次由 PPT 业务包自主切分；当模型明确输出超限时，按 PPT 对象边界拆半续审并合并结果，不依赖通用或 Word 审核引擎。先完成真实样本验收，再评估视觉版式、图片 OCR 和跨 PPT 审核。能读取或渲染 PPT 仍不等于已经能审核 PPT 版式。
