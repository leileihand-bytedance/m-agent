...
- 通过交互式关系图和状态清单查看用户入口、通用底座、业务功能、工具知识库、运维数据五层整体架构、能力关系和建设状态。
...
最近任务和用户权限共用按需敏感区。默认访问 `/` 时，服务端不会调用权限列表和最近任务读取函数，因此数据不只是视觉折叠，也不会出现在默认页面源码中。项目总览中的任务数量仍保留，因为它只统计固定任务目录下的元数据文件数量，不读取任务内容。
