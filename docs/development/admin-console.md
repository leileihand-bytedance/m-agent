...
- 查看用户入口、通用底座、业务功能、工具知识库、运维数据五层整体架构和各能力建设状态。
...
每个节点展示能力名称、当前建设状态、简要职责、事实依据和关联待办的下一动作。页面可以按以下状态筛选：
...
架构拓扑、节点职责和允许作为事实依据的代码路径维护在 `app/admin/services.py`。页面状态只组合以下可核对信息，不调用大模型判断完成度：
