...
3. 人工在真实企业微信中确认审核文件到达后只收到一次即时收件提示，单文件入队后不再追加队列或审核类型；分别用“格式审核”“帮我查一下格式”验证文件前置格式意图，确认不建立文字审核任务。纯文字审核完成后必须通过主动 Markdown 消息收到结果。正常提示不包含后台任务编号；模拟需要人工介入的异常时，用户提示包含“处理编号”。
