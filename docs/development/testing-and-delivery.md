...
2026-07-15 首轮 5 份真实文件的人工填写结果为 34 条有效、11 条误报、1 条未评分，46 条标注位置均为准确；建议质量和重要程度未完整填写，漏报补充区没有人工确认并补录的问题。8 条英文所有格弯撇号误报已用真实原文复核并补成离线回归，修复后均不再触发 `quote-pair`，真正缺失的中文引号仍会触发。后续基线比较必须把同根因重复误报单独归类，不能只看误报总条数。

2026-07-17 经用户确认，当前结果冻结为“通用审核质量基线 v1”，`TODO-023` 随之关闭。1 条未评分以及未完整填写的建议质量、重要程度不阻塞基线成立；漏报补充为空只能表述为“本轮无已确认补录漏报”，不能声称漏报率为零。同一批文件不为收口重复全量调用模型；后续发现新误报、漏报或定位错误时，优先增加脱敏等价回归和已保存结果回放，只有修改模型召回、分块或通篇逻辑行为时才做必要的定向真实模型测试。

2026-07-16 HTML 网页 PPT 实测发现模型把相邻但分属不同语法成分的字拼成一个错词。此类真实内容不写入仓库，使用脱敏等价句复现；回归必须同时验证上下文误报被删除、明确错别字经两次复核确认后仍保留、复核不可用时高风险错别字意见不交付。本轮全仓离线回归 `935` 项、审核联合回归 `153` 项、审核 Bot 独立保护测试 `9` 项通过；混合运行中的真实模型用例出现召回数量不足，按真实模型波动单独报告，不计入离线回归结论。

2026-07-17 单份 PPTX 企微实测发现名称审核缺少双边约束：模型把不同项目名称的大小写模式拿来比较，并把两页完全相同的数据脚注互相指认为问题。回归必须验证名称问题同时提供两处真实、不同且归一后同名的证据；单边名称、不同名称、相同脚注、全半角或圈号等脚注标记兼容变体、常见注释语句和反向重复候选均不交付，`C++ / c++` 等符号结尾名称仍保留。真实 20 页样本使用已保存候选本地回放后只保留明确占位符；隔离本轮差异后全仓离线回归 `1058` 项、PPT 专项与入口联合回归 `173` 项、PPT reviewer 专项 `36` 项和审核 Bot 独立保护测试 `9` 项通过。
