...
持久化执行器按任务类型分批接入。当前审核八类能力，以及直报、`writer1`、`research_synthesis`、`shenyinxie_news` 和 `internal_weekly` 已分别接入审核、写作专用 SQLite。多文件联合审核、综合调研、深银协动态与内参周报必须额外验证多个交付项的检查点：已经确认成功的摘要或附件不能因重启重复发送；进程在 `sending` 中断，或发送已发起但没有取得可判断回执时，必须转为 `delivery_unknown`、暂停自动重发并告警。综合调研要分别核对完成说明和 Word 初稿；内参周报初次生成只核对完成说明和 Markdown 核对稿，内部 JSON 溯源记录不得进入用户交付队列；人工批准后要核对完成说明和 Word 附件，且恢复交付不得重新搜索或调用模型。

每个任务类型接入时都必须完成：

1. 自动化验证同一企业微信消息重复投递只创建和执行一次任务。
2. 自动化验证全局、单用户和成本并发限制，排队期间 Bot 仍可受理其他消息。
3. 自动化验证综合调研、深银协动态和内参周报缺少必需附件时不发送成功提示，任务进入失败重试和运维告警；直报、简报纯文字模式不受影响。
4. 人工在真实企业微信中确认审核文件到达后只收到一次即时收件提示，单文件入队后不再追加队列或审核类型；分别用“格式审核”“帮我查一下格式”验证文件前置格式意图，再用“也做一下文字审核”“再做一下内容审核”验证文件后操作指令，确认这些指令本身都不建立纯文字审核任务。纯文字、HTML 和 PPTX 审核完成后必须通过主动 Markdown 消息收到结果；HTML/PPTX 结果只发消息、不回传标记文件。PPTX 还需确认不进入 Word 归集、不出现修改建议，旧版 `.ppt` 明确提示另存为 `.pptx`。正常提示不包含后台任务编号；模拟需要人工介入的异常时，用户提示包含“处理编号”。
5. 在生成过程中重启对应 Bot，确认任务恢复或安全失败，不会永久停在“处理中”。
6. 分别验证用户取消、模型或网络失败、正文已生成但附件发送失败，以及运维 Bot 告警。
7. 核对外部发送幂等：已经成功发送的结果不会因重试或重启再次发送，失败交付也不会触发重新生成两份稿件。
...
内参周报还要重点验证：完整周报恰好并行启动党政要闻、监管动态、同业动向、市场观察、前沿观点五个受控任务，并发上限为 5；汇总顺序始终固定，单板块异常不取消其他板块且整期不可批准，单独市场更新不启动另外四个任务。出版日固定为周一、统计期固定为上一自然周；党政、监管、同业和市场普通内容使用独立检索与筛选调用，五个板块标题始终按固定顺序输出，无法分类的材料不兜底到市场观察；同业实体表覆盖境内数字银行、国际/香港数字银行和国内银行科技子公司，微众自身排除；党建只保留党中央和金融监管部门内容。普通内容证据要由程序编号并取回原文块，摘要允许正常概括压缩，但改变数字、单位、政策题名或增减方向时必须被拦截；已通过价值筛选的材料要在同一原文上最多重生成 3 次，每次重新硬校验，连续失败时改用程序取回的原文证据作为保守摘录版，不得排除材料、保留错误摘要或输出“摘要待整理”。普通新闻正文必须以证据中的事件发生日或“近日”开头，发生日与网页发布日期分别记录且不得互相替代；模型遗漏日期时只允许从唯一明确的证据日期补取。前沿观点以核验后的报告发布日期开头，固定资本市场综述保留交易周期表达。同一大会、论坛等多来源条目必须综合成一个连贯段落，不能按来源逐段拼接。整批模型失败或连续空答时须保存本批候选标题。周一 15:00 前完整周报只跳过当日 A 股搜索并输出红色粗体占位，15:00 后包含 `weekly_a`、`monday_a`、`weekly_hk`、`weekly_us` 四组必填指数；单独的“今日资本市场综述”指令在收盘前不搜索，收盘后只请求 `monday_a`。行情涨跌幅、日期和原句均须硬校验；研报摘录逐字存在于来源页面。用户核对稿只显示出版日、统计期、正文和原文链接，内部溯源记录仍须包含 `draft_version`、URL、发生日期、发布日期、证据原句和正文哈希；核对稿全文哈希必须与任务元数据一致。初次生成和普通“生成 Word”不得产生 `.docx`；只有同一任务 `ready_for_approval=true`、核对稿日期与全文哈希一致且用户明确批准时才生成 Word。模板回归要确认五个板块顺序、Heading 1/3 和正文样式、水印、红线、页眉页脚、页码域、自动目录域、其他包部件和洁净内容；公共 Word 终稿回归还要覆盖目录暂存路径不使用任务原路径、固定脚本调用、超时清理、路径越界拒绝、Word 元数据净化、目录标题与批准稿完全一致、每项页码为正整数、`PAGEREF` 和 `_Toc` 书签有效。结构测试后必须逐页渲染，并用 Microsoft Word 确认成品打开时不出现文件访问或外部文件域更新警告，目录项和页码已经显示且无需人工更新。持久任务测试还必须覆盖重复消息只入队一次、处理中断后重新领取、生成完成后不重复运行 Skill，核对稿的完成说明和 Markdown 两项逐项交付、内部 JSON 不发送，以及批准后完成说明和 Word 的逐项交付；某项发送状态未知时不得自动重发已确认项目。真实联网验收要分别记录普通内容、行情和研报来源的公开标题与域名，不记录抓取全文；公开页面无法稳定返回历史行情时，应保留待核事项，不能把模拟测试通过解释为真实数据源已经稳定。
...
深银协专项回归还必须覆盖：标题含“微众”的多银行综合稿不能直接作为专题全文；完整专题稿不足时才启用扩展白名单；模型只可返回结构化全文/摘编/拒绝判断；摘编段落必须按原顺序逐字来自原文；高管到其他机构任职且微众成果仅为履历背景的稿件必须拒绝；Word 中摘编稿必须显示原报道标题、原文链接和摘编说明。模板回归还要校验 A4 页面和页边距、标题/动态标题/正文角色样式、逐段正文、未使用块删除、网页页头及固定站点尾部推广清理、入选稿繁转简、正文段首信源标记仅该标记为红色、中文发布日期、外部超链接关系以及无真实案例正文残留；结构测试通过后必须逐页渲染检查重叠、截断和异常空页。真实联调只记录标题、域名和内容模式，不记录抓取正文。
