...
内参周报还要重点验证：完整周报恰好并行启动党政要闻、监管动态、同业动向、市场观察、前沿观点五个受控任务，并发上限为 5；汇总顺序始终固定，单板块异常不取消其他板块且整期不可批准，单独市场更新不启动另外四个任务。出版日固定为周一、统计期固定为上一自然周；党政、监管、同业和市场普通内容使用独立检索与筛选调用，五个板块标题始终按固定顺序输出，无法分类的材料不兜底到市场观察；同业实体表覆盖境内数字银行、国际/香港数字银行和国内银行科技子公司，微众自身排除；党建只保留党中央和金融监管部门内容。普通内容证据要由程序编号并取回原文块，摘要允许正常概括压缩，但改变数字、单位、政策题名或增减方向时必须被拦截；已通过价值筛选的材料要在同一原文上最多重生成 3 次，每次重新硬校验，连续失败时改用程序取回的原文证据作为保守摘录版，不得排除材料、保留错误摘要或输出“摘要待整理”。普通新闻正文必须以证据中的事件发生日或“近日”开头，发生日与网页发布日期分别记录且不得互相替代；模型遗漏日期时只允许从唯一明确的证据日期补取。前沿观点以核验后的报告发布日期开头，固定资本市场综述保留交易周期表达。同一大会、论坛等多来源条目必须综合成一个连贯段落，不能按来源逐段拼接。整批模型失败或连续空答时须保存本批候选标题。周一 15:00 前完整周报只跳过当日 A 股搜索并输出红色粗体占位，15:00 后包含 `weekly_a`、`monday_a`、`weekly_hk`、`weekly_us` 四组必填指数；单独的“今日资本市场综述”指令在收盘前不搜索，收盘后只请求 `monday_a`。行情涨跌幅、日期和原句均须硬校验；研报摘录逐字存在于来源页面；核对稿和溯源清单的 `draft_version` 一致，每个来源都有 URL、发生日期、发布日期、证据原句和正文哈希。初次生成和普通“生成 Word”不得产生 `.docx`；只有同一任务 `ready_for_approval=true`、核对稿日期与 `draft_version` 一致且用户明确批准时才生成 Word。模板回归要确认五个板块顺序、Heading 1/3 和正文样式、水印、红线、页眉页脚、页码域、自动目录域、其他包部件和洁净内容；公共 Word 终稿回归还要覆盖目录暂存路径不使用任务原路径、固定脚本调用、超时清理、路径越界拒绝、Word 元数据净化、目录标题与批准稿完全一致、每项页码为正整数、`PAGEREF` 和 `_Toc` 书签有效。结构测试后必须逐页渲染，并用 Microsoft Word 确认成品打开时不出现文件访问或外部文件域更新警告，目录项和页码已经显示且无需人工更新。持久任务测试还必须覆盖重复消息只入队一次、处理中断后重新领取、生成完成后不重复运行 Skill，核对稿的完成说明、Markdown 和 JSON 三项逐项交付，以及批准后完成说明和 Word 的逐项交付；某项发送状态未知时不得自动重发已确认项目。真实联网验收要分别记录普通内容、行情和研报来源的公开标题与域名，不记录抓取全文；公开页面无法稳定返回历史行情时，应保留待核事项，不能把模拟测试通过解释为真实数据源已经稳定。
