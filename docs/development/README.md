...
- CLI 配置检查。
- 本机项目控制台：保留 Skill 开关、用户权限和最近任务，自动展示五层整体架构、功能建设状态、六个板块进展、开放 TODO、最近 Git 更新、知识库/任务数量和 Bot 心跳。

后续重点：

1. 先按 `TODO-023` 建立通用审核真实样本回归和质量基线，收敛误报、漏报、长文逻辑和标注位置。
2. 已完成 `TODO-019` 独立公文格式审核；该模式仅显式触发，不接入普通内容审核。
3. `TODO-021` 已实现 `.docx` 多文件联合审核 v1，先用真实正文和附件完成质量与回传验收；随后继续把写作、审核两套持久化组装下沉为 `TODO-003` 公共能力。
...
- 联合审核支持 2 至 5 份 `.docx`，不按上传顺序默认正文；文件名证据不唯一时要求用户指定主文件，再执行逐文件审核、附件确定性检查和双边原文证据的跨文件语义检查。
...
- 项目控制台已增加五层架构与功能状态图，可按稳定运行、上线优化、建设中、待建设、暂缓和关闭筛选；建设成熟度与 Bot 当前在线状态分开显示。
