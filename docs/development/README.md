...
3. 把文件接收和多消息任务组装下沉为底座公共能力，再按 `TODO-021` 支持“正文 + 多附件”联合审核。
4. 完成公共大文件回传后，再推进 PPT 审核；避免内容审完但结果无法稳定交付。
...
3. 按 `TODO-003` 沉淀公共文件接收和多消息任务组装，再按 `TODO-024` 建立统一文档服务，然后实施 `TODO-021` 多文件联合审核。
4. 按 `TODO-017` 验证大文件和多结果回传，再推进 `TODO-020` PPT 审核；HTML 文件支持暂缓。
