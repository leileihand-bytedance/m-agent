...
2. 基线稳定后按 `TODO-019` 增加通用 Word 格式审核。
...
- 审核 Bot 继续独立运行，已支持内参周报、半月报和通用 Word/文字审核，不急于迁移到统一入口。
- 通用审核已具备代码规则、分段语义审核、`200` 至 `100000` 字通篇逻辑校对、长文双扫描候选复核和内容匹配优先的 Word 标注流程。
- 2026-07-13 审核专项离线自动化测试 `200` 项通过；当前缺口是经授权真实文件回归集，以及通用 Word 格式、多文件联合和 PPT 审核。
...
2. 基线稳定后推进 `TODO-019`，补通用 Word 格式审核。
