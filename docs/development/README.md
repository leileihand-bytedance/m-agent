...
- 已将审核 Bot 原有用户名注册表抽到 `app/platform/user_registry.py`，写作 Bot 和审核 Bot 共用 `data/review_users.yaml`。
- 写作 Bot 的即时提示已改为按实际 skill 动态变化：直报、单素材简报、多素材简报不再共用“直报写作流程”话术。
- 已新增独立运维 Bot：异常和链接读取失败等事件写入 `data/platform/ops_events/`，运维 Bot 独立长连接发送告警，并在每个工作日 9:00 发送前一工作日日报；写作 Bot 和审核 Bot 已接入心跳监控。
