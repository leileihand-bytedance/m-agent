...
3. 统一文档服务 v1 已完成；继续把写作入口的持久化多消息任务组装下沉为公共能力，再按 `TODO-021` 支持“正文 + 多附件”联合审核。
...
- 审核 Bot 继续独立运行，已支持内参周报、半月报、通用 Word/文字审核，以及用户明确要求后执行的一次性公文格式审核，不急于迁移到统一入口。
- 通用审核已具备代码规则、分段语义审核、`200` 至 `100000` 字通篇逻辑校对、长文双扫描候选复核和内容匹配优先的 Word 标注流程。
- 2026-07-13 审核专项离线自动化测试 `212` 项通过；独立公文格式审核已用桌面真实模板验证零问题，当前缺口是经授权真实文件内容审核回归集，以及多文件联合和 PPT 审核。
...
3. `TODO-024` 统一文档服务 v1 已完成；按 `TODO-003` 把持久化多消息任务组装下沉为公共入口，再实施 `TODO-021` 多文件联合审核。
