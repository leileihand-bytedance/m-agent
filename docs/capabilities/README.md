...
| 内参周报 | `skills/internal_weekly/` | 搜集上一自然周公开信息，先输出正文加原文链接的精简核对稿，后台保留结构化溯源，人工明确批准同一版本后生成洁净版 Word |
...
直报、简报和材料润色还共用底座 `RevisionEngine`：统一识别标题、指定段落或全文范围，保护未点名内容，并检查篇幅、数字保持和指定删除项。各 Skill 仍独立负责自己的文种结构、语言和质量规则。
...
包含文种判断的共享写作规划和质量检查可以放在 `skills/` 顶层公共模块，但不能绕过底座工具授权。跨文种的任务关系、版本选择和改稿执行协议属于 `app/platform/`，不得在各 Skill 中复制实现。
