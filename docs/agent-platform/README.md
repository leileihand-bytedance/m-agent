...
公共文件与文档处理继续统一使用桌面 `M-Agent-Files/`：尚未开始处理的文件暂存在 `runtime/intake/`；任务原件进入 `input/`；PDF/PPT 的解析结果、渲染图片和 OCR 等中间产物进入 `work/`；最终报告、标注或生成文件进入 `output/`。这些运行文件都不进入 Git。

近期格式范围是 Word、PDF 和后续 PPT。HTML 文件支持已暂缓；现有网页链接读取能力保持不变，不因本轮文档服务建设扩大为 HTML 文件上传或执行能力。
...
下一步先把写作 Bot 内已经验证的文件消息和短任务组装能力下沉为底座公共能力，并补齐多文件暂存、超时、显式开始、取消和任务隔离，让现有写作 Bot 与审核 Bot 都能复用；随后建设统一文档服务，先解决 PDF 按页解析、长文分块和标准材料结构，再为 PPT 复用。底座只负责组装和解析本次任务，正文与附件一致性等规则仍由审核模块负责。HTML 文件支持暂缓，统一企业微信入口继续作为长期方向，但都不作为当前审核质量和多文件审核的前置条件。
