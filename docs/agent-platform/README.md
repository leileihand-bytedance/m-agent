...
当前写作 Bot 已经通过 `app/writing/bot.py` 接入真实 AiBotSDK 长连接，并调用 `PlatformApp`。材料润色另设 `app/rewrite_bot/` 作为过渡期独立入口，但业务逻辑仍只保留在 `skills/rewrite/`，底座、权限和工具层全部复用。长期方向仍是统一入口，不把每个新 Skill 都复制成一套独立业务实现。
...
- CLI 配置检查和本地消息入口。
- 写作 Bot 真实企业微信文本入口。
- 独立材料润色 Bot 入口：注册表白名单固定为 `rewrite`，只接收直接粘贴文字，文件和链接明确拒绝；任务和会话目录与原写作 Bot 隔离。微众相关原文可通过 Skill 自身的关联闸门读取本地 `bank_materials` 做名称和标准表述核对，普通材料不检索，且不得借语料新增事实。
