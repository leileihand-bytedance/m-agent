...
统一文档服务、公共任务协议、公共附件交付和持久化执行器内核已经完成。它们分别解决收材料、跑任务、读文件和发结果的问题；其中任务执行器尚未承载全部现有流量。下一步先把直报作为生产试点，确认快速受理、重启恢复、重复消息、取消、排队和结果交付均可靠，再扩展到其他写作任务。审核 Bot 暂时保持独立。底座只负责组装、执行、解析和交付本次任务，正文与附件一致性、PPT 版式等业务规则仍由审核模块负责。HTML 文件支持暂缓，统一企业微信入口继续作为长期方向。
