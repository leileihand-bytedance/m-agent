...
写作状态机由 `app/writing/intake.py` 实现，审核状态机由 `app/review/intake.py` 实现；两者已经共同复用 `app/platform/intake.py`。公共层统一匿名用户目录、原子状态、文件暂存、重启恢复、TTL 清理、防目录外文件引用、文件数量和总大小校验。具体“写什么”“是否格式审核”“哪份是正文”仍留在各模块。后续要继续统一材料、等待、开始、取消和任务提交协议，而不是再次复制持久化代码。
...
- `document_reader`：已实现，统一安全读取 DOCX、PDF 和 PPTX，返回适合模型使用的标准材料片段，同时把完整解析结果写入任务 `work/documents/`。DOCX 内嵌图片不做 OCR，在原位置生成可传给模型的人工评估提醒；长材料抽样会强制保留提醒。写作 skill 已优先使用该工具，旧 Word/PDF 工具保留兼容。
...
公共文件与文档处理继续统一使用桌面 `M-Agent-Files/`：尚未开始处理的文件暂存在 `runtime/intake/`；任务原件进入 `input/`；DOCX/PDF/PPTX 的完整解析结果进入 `work/documents/`，DOCX/PPT 提取图片进入对应 `assets/`；后续渲染图片和 OCR 等中间产物也进入 `work/`；最终报告、标注或生成文件进入 `output/`。这些运行文件都不进入 Git。
...
- 跨写作和审核复用的底座级多文件业务组装协议；公共持久化内核已经完成。
- 持久化后台任务执行与恢复，包括消息幂等、并发限制、取消和重启续跑。
...
统一文档服务 v1 已解决 DOCX/PDF/PPTX 的公共安全解析、标准材料结构、长材料均衡取样和完整结果落盘；任务暂存的公共安全内核也已下沉。下一步统一写作和审核的业务组装协议，并补持久化后台执行、公共附件回传、扫描 PDF 按需 OCR、页面渲染和复杂文件回归。底座只负责组装、执行和解析本次任务，正文与附件一致性、PPT 版式等业务规则仍由审核模块负责。HTML 文件支持暂缓，统一企业微信入口继续作为长期方向。
