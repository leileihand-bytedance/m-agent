...
- 对已切换的五类单项审核、直报、`writer1`、`writer2` 完成真实重启、重复消息、取消、多用户排队和发送幂等验收。
...
统一文档服务、公共任务协议、公共附件交付和持久化执行器内核已经完成。它们分别解决收材料、跑任务、读文件和发结果的问题；五类单项审核、直报和两类简报已经接入生产 handler，当前进入快速受理、重启恢复、重复消息、排队和结果交付的真实验收。审核 Bot 仍保持独立入口，多文件联合审核没有切流。底座只负责组装、执行、解析和交付本次任务，正文与附件一致性、PPT 版式等业务规则仍由审核模块负责。HTML 文件支持暂缓，统一企业微信入口继续作为长期方向。
