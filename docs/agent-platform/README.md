...
下一步先把写作 Bot 内已经验证的文件消息和短任务组装能力下沉为底座公共能力，并补齐多文件暂存、超时、显式开始、取消和任务隔离，让现有写作 Bot 与审核 Bot 都能复用。底座只负责组装本次任务，正文与附件一致性等规则仍由审核模块负责。统一企业微信入口继续作为长期方向，但不作为当前审核质量、格式审核和多文件审核的前置条件。
