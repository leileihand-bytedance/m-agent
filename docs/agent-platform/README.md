...
- 写作、审核重任务按类型切换到持久化执行器并完成真实重启验收；执行器存在不等于当前所有 Bot 已自动续跑。
...
统一文档服务、公共任务协议、公共附件交付和持久化执行器内核已经完成。下一步不是继续补一套底座，而是按任务类型把写作、审核和复杂文档处理切换到执行器，并用真实重启、重复消息和大文件交付验收。底座只负责组装、执行、解析和交付本次任务，正文与附件一致性、PPT 版式等业务规则仍由审核模块负责。HTML 文件支持暂缓，统一企业微信入口继续作为长期方向。
