...
该能力由 `app/writing/intake.py` 实现，默认 1800 秒过期。状态和文件持久化到 `M-Agent-Files/runtime/intake/`，Bot 重启后可恢复有效会话，任务提交或过期后清理。审核 Bot 也已通过 `app/review/intake.py` 实现模块内的格式审核衔接和 `.docx` 连续文件自动组装：文件到达即确认，1 份自动走单文件审核，2 至 5 份自动走联合审核；后续仍需把两套接口下沉为 `app/platform/` 公共入口能力，供跨 Bot 和多进程部署复用。
...
- CLI 配置检查和本地消息入口。
- 写作 Bot 真实企业微信文本入口。
- 写作 Bot 短任务组装 v1：支持链接、文字、Word/PDF/PPTX 跨消息组成一次直报/简报请求，也支持把 1 份现成提纲和多部门文件组成一次综合调研整合请求；待组装文件可在 Bot 重启后恢复。`rewrite` v1 只接收直接粘贴文字，并支持“原文在前、润色要求在后”的新任务写法，不会误接到上一条直报/简报改稿。
- 审核 Bot 短任务组装 v1：格式审核支持文件和指令两种顺序；直接发送 1 份 `.docx` 自动走单文件审核，连续发送 2 至 5 份自动走联合审核，无需前后指令；保留取消、主文件确认和持久化恢复，不按上传顺序默认正文。
- 本机管理后台。
- 搜索工具。

仍未完成：

- 跨写作和审核复用的底座级多文件会话组装接口；两个旧入口已有各自实现，但尚未完成公共抽象。
...
统一文档服务 v1 已解决 DOCX/PDF/PPTX 的公共安全解析、标准材料结构、长材料均衡取样和完整结果落盘。审核 Bot 已先行实现 `.docx` 联合审核 v1；下一步用真实多文件材料验收，再把写作和审核各自的持久化短任务组装下沉为底座公共能力，并补扫描 PDF 按需 OCR、页面渲染和复杂文件回归。底座只负责组装和解析本次任务，正文与附件一致性、PPT 版式等业务规则仍由审核模块负责。HTML 文件支持暂缓，统一企业微信入口继续作为长期方向。
