...
1. 先用审核 Bot 内的单个 Word 通用审核验证持久任务执行，不改变独立入口。
2. 再迁移直报写作。
3. 再迁移简报和综合调研写作。
4. 其他审核能力逐类评估，不整体切换。
...
统一文档服务、公共任务协议、公共附件交付和持久化执行器内核已经完成。它们分别解决收材料、跑任务、读文件和发结果的问题；单个 Word 通用审核现已作为首个生产 handler 接入，先确认快速受理、重启恢复、重复消息、排队和结果交付均可靠，再扩展到直报和其他写作任务。审核 Bot 仍保持独立入口，内参、半月报、公文格式和多文件审核没有切流。底座只负责组装、执行、解析和交付本次任务，正文与附件一致性、PPT 版式等业务规则仍由审核模块负责。HTML 文件支持暂缓，统一企业微信入口继续作为长期方向。
