...
该能力由 `app/writing/intake.py` 实现，默认 1800 秒过期。状态和文件持久化到 `M-Agent-Files/runtime/intake/`，Bot 重启后可恢复有效会话；任务需要澄清时继续保留，用户回答后自动续跑，成功、失败、取消或过期后再清理。审核 Bot 也已通过 `app/review/intake.py` 实现模块内的格式审核衔接和 `.docx` 连续文件自动组装：文件到达即确认，1 份自动走单文件审核，2 至 5 份自动走联合审核；后续仍需把两套接口下沉为 `app/platform/` 公共入口能力，供跨 Bot 和多进程部署复用。
