...
- 持久任务业务接入：纯文字、单个通用 Word、内参、半月报、公文格式使用审核专用队列；直报、`writer1`、`writer2` 使用独立写作队列。审核输入在入队前冻结到任务目录，写作任务入队前创建正式 job；两边都按处理和主动发送检查点恢复。
- 文档增强：扫描 PDF 按需 OCR，PDF/PPTX 显式页面渲染和资源限制已完成。

仍未完成：

- 对已切换的六类单项审核（含静态 HTML）、直报、`writer1`、`writer2` 完成真实重启、重复消息、取消、多用户排队和发送幂等验收。
...
统一文档服务、公共任务协议、公共附件交付和持久化执行器内核已经完成。它们分别解决收材料、跑任务、读文件和发结果的问题；六类单项审核、直报和两类简报已经接入生产 handler，当前进入快速受理、重启恢复、重复消息、排队和结果交付的真实验收。审核 Bot 仍保持独立入口，多文件联合审核没有切流。底座只负责组装、执行、解析和交付本次任务，正文与附件一致性、PPT 版式等业务规则仍由审核模块负责。独立审核入口已支持单个静态 HTML；写作 HTML、动态渲染、视觉版式和多文件联合仍暂缓，统一企业微信入口继续作为长期方向。
