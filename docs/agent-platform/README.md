...
该能力由 `app/writing/intake.py` 实现，默认 1800 秒过期。状态和文件持久化到 `M-Agent-Files/runtime/intake/`，Bot 重启后可恢复有效会话，任务提交或过期后清理。审核 Bot 也已通过 `app/review/intake.py` 实现模块内的格式审核衔接和 `.docx` 联合审核组装；后续仍需把两套接口下沉为 `app/platform/` 公共入口能力，供跨 Bot 和多进程部署复用。
...
- 审核 Bot 短任务组装 v1：格式审核支持文件和指令两种顺序；联合审核支持 2 至 5 份 `.docx`、显式开始、取消、主文件确认和重启恢复，不按上传顺序默认正文。
- 本机管理后台。
- 搜索工具。
