...
该数据库属于本机资料库，可能包含内部或敏感材料，已加入 `.gitignore`，不要提交。

当前已导入语料经用户确认均为公开信息，并授权 `rewrite` 在微众相关材料润色时，把每次最多 3 条相关短摘录发送给当前配置的 DeepSeek，仅作口径复核。该授权不自动覆盖新导入材料；当前条目元数据尚无公开/内部可见性字段，未分类的新材料不能视为已获外发授权。
...
3. 输出材料包必须保留来源文件和页码。
4. 当前尚未自动识别来源优先级或口径冲突，模型不得自行选择冲突数据。

来源可见性、优先级、冲突检测和标准表述治理统一维护在 `docs/development/TODO.md` 的 `TODO-010`，不在本文重复维护建设进度。
