...
def test_search_queries_include_exact_publication_period_and_are_staged():
    ...
    assert any("上海证券报" in query and "每日经济新闻" in query for query in primary)
    assert any("AI原生" in query and "算力" in query and "智能体" in query for query in primary)
    assert any("专利" in query and "知识产权" in query for query in primary)
