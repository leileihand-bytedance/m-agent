...
`python tests/test_reviewer.py` 包含真实 LLM 端到端测试，可能因网络或模型连接失败。失败时要说明原因，不要误判为代码必然坏了。
...
3. 完成后运行 `python scripts/project_docs.py check`；提交前由 Git hook 自动运行 `python scripts/project_docs.py check --staged`，按底座、写作、审核、skill、配置、依赖/交付机制分别核对对应文档，任意计划文档不能代替核心文档。
...
3. 推送前先执行 `git fetch origin`，确认远端没有未合并的新提交；禁止使用 `--force` 覆盖远端历史。
4. 本地领先且远端未分叉时，及时推送当前 `main`；推送后运行 `python scripts/project_docs.py check-sync`，必须显示本地与远端已同步。
5. 如果网络、权限或远端分叉导致无法推送，必须明确报告，不能把“已本地提交”说成“已同步远端”。
6. post-commit hook 只负责提醒尚未推送，不自动执行网络操作；pre-push hook 会再次检查项目文档。这样既能及时同步，也不会静默强推或覆盖他人提交。
