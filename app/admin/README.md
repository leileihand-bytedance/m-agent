...
- 通过简化的信息流图查看“入口 -> 底座 -> 写作/审核 -> 结果交付”，并通过独立状态清单查看建设状态、在线状态及“持久队列/实时执行”方式。
- 查看六个板块的最新情况和首要待办。
...
- 架构图只展示业务入口、底座的接入/编排/运行三层、写作、审核、结果交付和运维可观测性八个高层节点，不再为每一项功能绘制节点和连线。
- 写作、审核分别使用绿色和蓝色区域，并在区域内用文字概括直报、简报、续写改稿、专题内容及通用/专项/格式/多文件审核。共享工具、知识库和底座治理能力同样以文字说明，避免遗漏未单列能力。
...
架构节点、关系、状态组件和证据路径维护在 `app/admin/services.py`。主图使用固定分区和带圆角的直角路径，流动虚线示意请求与结果传递；写作路线为绿色，审核路线为蓝色，可暂停动效，系统减少动态效果时自动禁用动画。节点仍可点击查看详情，窄屏仅在图内横向浏览。状态清单可按成熟度筛选，实际状态由 TODO、Skill 开关、相关代码是否存在和 Bot 心跳组合生成。建设成熟度、当前在线状态和执行方式分开显示；执行方式直接读取写作与审核任务类型的真实注册状态，不使用模型或手工进度文字判断。
