...
架构节点、关系和证据路径维护在 `app/admin/services.py`，实际状态由 TODO、Skill 开关、相关代码是否存在和 Bot 心跳组合生成。当前架构图把通用文字、通用 Word、静态 HTML、内参、半月报、公文格式、多文件和单份 PPTX 审核分别作为独立节点；共享审核核心作为这些节点的公共依赖，不算第九类审核功能。关系图支持状态筛选、缩放、拖动、点击节点查看详情，并保留状态清单作为备用视图。建设成熟度与当前在线状态分开显示，不使用模型自由判断完成度。
