...
PPT 低级错误审核已经作为独立业务包接入企业微信审核 Bot。`app/review/ppt/` 从底座统一文档服务取得任务内 `.pptx` 的可编辑文本框、表格和图表文字，并排除图片文字和演讲者备注；PPT 自己维护占位符、成对标点、连续标点和按段落层级分组的序号跳号/重复/倒序规则。独立模型审核负责发现语言问题和单份 PPT 内部一致性候选，所有候选必须经过页码、对象和逐字原文校验；名称写法不一致必须同时命中同一专有名称的两种不同原文写法，代码只在忽略大小写、宽窄字符和名称内常见分隔符后能确认是同一名称时保留。单个名称、不同名称、相同重复脚注、以脚注或列表标记开头的候选及常见注释语句会被丢弃，`C++` 这类符号结尾名称不受影响。跨页候选除模型声明外，还会由代码拦截明确不同的年份、时间范围、单位和“目标/实际”等口径，反向重复的双边候选只保留一条。该包不导入通用审核、Word 审核或其他业务审核引擎。用户看到的问题说明完全由代码按类别生成，不透传模型自由描述，因此模型无法夹带修改建议。语言审核常规批次限制为约 3000 字符；当模型明确因输出上限停止时，按 PPT 对象边界自动拆半审核并合并结果，不再把被截断的 JSON 当成整批失败。每个成功的顶层模型批次单独写入任务断点，重启只补未完成批次。结果通过持久任务按顺序发送纯文字，不生成或回传 Word、PPT 附件。

2026-07-17 已完成首份经授权 20 页真实 PPT 的端到端实测，并针对实测暴露的名称误报补齐双边证据约束。使用已保存候选本地回放后，原样本只保留第 1 页明确占位符；隔离本轮差异后，全仓离线回归 `1058` 项、PPT 专项与入口联合回归 `173` 项、PPT reviewer 专项 `36` 项和审核 Bot 独立保护测试 `9` 项通过。该结论只证明已知误报已被拦截，更多真实样本的漏报、页码、图表读取和企业微信长结果展示仍需继续验收。
...
结果不提供修改建议，也不判断矛盾两处中的哪一处正确。模型返回的自由问题说明不会进入最终结果，“问题”一栏只使用代码维护的类别事实说明。较长结果按问题编号连续分段发送。语言审核按批次保存断点，任务重启时不会重新调用已完成批次。常规批次约 3000 字符；如果 DeepSeek 明确返回 `stop_reason=max_tokens`，程序只对这一批按对象拆半后继续，并合并两边的 `issues`。网络异常、普通 JSON 格式错误和单个对象仍输出超限不会被当成可拆分情况，以免掩盖真实故障或无限重试。图表底层数据未能完整读取时，结果会列出具体页码的读取提示，不能把“已读取范围内未发现问题”表述成整份 PPT 完整通过。
...
- `20000` 字及以上文档会对名称、语病、标点、重复、内容不完整和跨段逻辑等高误报风险候选并行复核两次；候选至少经过首轮识别和一次独立复核确认才保留。模型生成的错别字候选不再因“目标短片段确实存在”就直接保留，而是无论篇幅都带完整段落和前后文进入两次严格语义复核，只有两次都确认是明确错误才输出；代码词表等确定性错字仍直接保留。
...
- 错别字提示词和规则库统一使用“原文错误片段应为正确写法”，修改前文本必须与 `target_text` 一致；普通分段审核使用零温度，输出前要求再次核对原文。错别字判断必须先分析完整句子的主干、修饰语和并列关系，不能把相邻但分属不同语法成分的字拼成错词。模型错别字候选和原文证据矛盾候选都会进入两次高精度复核且要求一致确认；复核不可用时不交付这类高风险意见。
...
2026-07-17 已将首批 5 份真实文件结果冻结为“通用审核质量基线 v1”：46 条候选中 34 条有效、11 条误报、1 条未评分，46 条标注位置均准确；主要误报根因已经修复并补入离线回归。漏报补充区没有人工确认并补录的问题，只能记录为“本轮无已确认补录漏报”，不能表述为漏报率为零。1 条未评分以及未完整填写的建议质量、重要程度不阻塞基线成立；以后只在出现新问题时增加脱敏回归、结果回放或必要的定向真实测试，不为收口全量重跑同一批文件。
