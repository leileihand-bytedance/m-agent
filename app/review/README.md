...
- `M_AGENT_MODEL_TIMEOUT_SECONDS`、`M_AGENT_MODEL_MAX_ATTEMPTS`、`M_AGENT_MODEL_RETRY_BACKOFF_SECONDS`：底座模型策略；当前审核器保留各阶段已有的请求超时和调用预算，但共用安全错误分类。
...
- 审核模型异常统一转换为底座安全错误码；鉴权错误立即停止，可重试错误只消耗当前审核阶段的既有预算，任务执行层不会再次从头重跑整项审核。
