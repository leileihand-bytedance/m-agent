...
PPT 低级错误审核正在按独立业务包推进。当前已建立 `app/review/ppt/` 的任务内安全提取结构，可从底座统一文档服务取得可编辑文本框、表格和图表文字，并排除图片文字和演讲者备注；PPT 自己维护的占位符、成对标点、连续标点和同一对象内序号跳号/重复/倒序规则也已具备。企业微信 `.pptx` 入口、独立模型审核和文字结果交付尚未接通，因此当前仍不能对外表述为 PPT 审核可用。
