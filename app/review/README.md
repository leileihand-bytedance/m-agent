...
7. 存档到 `data/reviews/2026-06-13-001/`
...
6. 企业微信不再发送"错误1/错误2..."文字列表，直接返回文档：
   - 有问题时返回 `marked_原文件名.docx`
   - 无问题时不回传文档，只回复“没有发现问题，可以走审批了”
7. 存档到 `data/reviews/YYYYMMDD-NNN/`
...
6. 原始文件和实际生成的 marked 文件存档到 `data/reviews/YYYYMMDD-NNN/source/`。
...
4. 把原始文字、审核意见和段落预览一起存档到 `data/reviews/YYYYMMDD-NNN/`
5. 以纯文本形式返回审核意见

限制:

- 文字消息**不会生成 marked 文档**,只返回文本意见，但现在会保存 `source/文字消息.txt`、`report.md` 和 `meta.md`，便于事后复盘
...
- `M_AGENT_LOGS_DIR`: 日志根目录,默认 `data/logs`
...
- 文件解析失败、审核失败、标注文档生成失败、结果发送失败都会写入 `data/platform/ops_events/`。
...
- 如果后续仍出现大文件上传回执超时，可先查看 `data/logs/review-bot-YYYY-MM.log` 中实际回执延迟，再谨慎调大该值。
...
2. 用户发送名字（支持中文、英文、数字、下划线、短横线，2-30 字符）→ 记录到 `data/review_users.yaml`
...
本机预注册用户写入 `data/review_users.yaml`，格式示例：
...
`data/review_users.yaml` 现在也是写作 Bot 使用的共享用户名表，写作任务记录、会话记录和开发期对话日志会同步保存 `sender_name`。该文件包含企业微信用户 ID，属于本机运行数据，已加入 `.gitignore`。
