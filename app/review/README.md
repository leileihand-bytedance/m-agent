...
当前五类单项审核已经接入审核专用持久任务执行器：纯文字、单个通用 Word、内参、半月报和公文格式。Bot 对普通文件仍先等待 8 秒完成单/多文件分流；确认是单项审核后返回对应审核类型的已受理提示，由后台 worker 调用原有对应审核引擎并主动发送结果。后台始终保留任务编号，正常受理和重复提交提示不向用户展示。多文件联合审核仍保持原执行方式，尚未迁移到持久任务执行器。
...
两种顺序最终都会冻结一份独立文件快照并返回“公文格式审核”已受理提示，由审核 worker 后台处理；不会与已经开始的内容审核共用检查点。
