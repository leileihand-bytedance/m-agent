...
`app/review/` 是 M-Agent 的旧审核企业微信 Bot，支持三种自动识别的内容审核类型，以及一种需要用户明确提出的独立格式审核：
...
4. **公文格式审核**：用户先说“格式审核”“按公文格式检查”等明确要求，再发送 `.docx`；只检查实际格式，不审核文字内容。
...
公文格式审核是例外：它不靠文件名或文档内容自动识别，只在当前用户明确提出格式审核后，对其下一份有效 `.docx` 执行一次。

## 独立公文格式审核

### 触发方式

1. 用户先发送“格式审核”“审核公文格式”“按公文格式检查”等短指令。
2. Bot 提示用户发送或重新发送 `.docx`。
3. 用户下一份有效 `.docx` 只做格式审核，不再同时做通用文字审核。
4. 该模式为一次性状态，使用后清除；默认超过 30 分钟未发文件也会失效。
