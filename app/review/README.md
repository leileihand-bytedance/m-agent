...
1. 用户直接发送 `.docx`，不需要事前说“联合审核”，也不需要发完说“开始审核”。每份文件到达后 Bot 都会立即确认，不会等自动归集完成才回复。
...
- 待组装状态默认保留 30 分钟，持久化在 `M-Agent-Files/runtime/intake/review/`，按入口和用户隔离；Bot 重启后文件不会丢失，后续文件或明确开始指令可继续已有批次。
