...
- 模型意见如果明确写成“原文 X 应为 Y”，代码会核对 X 是否真实出现在对应原文、X 与 Y 是否确实不同；只有修改后文本存在、模型却虚构修改前文本的意见会直接丢弃，例如不能把原文 `7×24小时` 说成 `7×7小时`。
- `general-incomplete` 会保守排除 `Fortune` 等短小、标签形态的纯英文段落，避免把表格中的机构名、奖项来源或栏目名当成截断正文；普通英文句子仍按原流程审核。
