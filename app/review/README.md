...
结果不提供修改建议，也不判断矛盾两处中的哪一处正确。模型返回的自由问题说明不会进入最终结果，“问题”一栏只使用代码维护的类别事实说明。较长结果按问题编号连续分段发送。语言审核按批次保存断点，任务重启时不会重新调用已完成批次；图表底层数据未能完整读取时，结果会列出具体页码的读取提示，不能把“已读取范围内未发现问题”表述成整份 PPT 完整通过。
