...
PPT 低级错误审核已经作为独立业务包接入企业微信审核 Bot。`app/review/ppt/` 从底座统一文档服务取得任务内 `.pptx` 的可编辑文本框、表格和图表文字，并排除图片文字和演讲者备注；PPT 自己维护占位符、成对标点、连续标点和按段落层级分组的序号跳号/重复/倒序规则。独立模型审核负责发现语言问题和单份 PPT 内部一致性候选，所有候选必须经过页码、对象和逐字原文校验；跨页候选除模型声明外，还会由代码拦截明确不同的年份、时间范围、单位和“目标/实际”等口径。该包不导入通用审核、Word 审核或其他业务审核引擎。模型说明和最终格式化均会清除修改建议，只保留事实问题；每个成功的模型批次单独写入任务断点，重启只补未完成批次。结果通过持久任务按顺序发送纯文字，不生成或回传 Word、PPT 附件。
...
每条模型候选必须能在指定页、指定对象中逐字找到原文；跨页问题还必须同时命中两处原文，并明确主体、时间和指标口径相同。即使模型误称口径一致，代码仍会拦截原文中明确不同的年份、季度或半年、单位，以及“目标/计划”和“实际/已完成”等状态。模型伪造的页码、对象或原文会被丢弃。一致性阶段单独失败时，已经完成的文字检查仍保留，结果会明确说明全篇一致性检查未完成。
...
结果不提供修改建议，也不判断矛盾两处中的哪一处正确。模型即使夹带“建议修改为”等话术，证据层和最终格式化层也会再次清除。较长结果按问题编号连续分段发送。语言审核按批次保存断点，任务重启时不会重新调用已完成批次；图表底层数据未能完整读取时，结果会列出具体页码的读取提示，不能把“已读取范围内未发现问题”表述成整份 PPT 完整通过。
