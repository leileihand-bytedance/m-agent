...
PPT 低级错误审核已经作为独立业务包接入企业微信审核 Bot。`app/review/ppt/` 从底座统一文档服务取得任务内 `.pptx` 的可编辑文本框、表格和图表文字，并排除图片文字和演讲者备注；PPT 自己维护占位符、成对标点、连续标点和同一对象内序号跳号/重复/倒序规则。独立模型审核负责发现语言问题和单份 PPT 内部一致性候选，所有候选必须经过页码、对象和逐字原文校验，跨页候选还必须同时满足主体、时间、指标口径一致；该包不导入通用审核、Word 审核或其他业务审核引擎。审核结果只含页码、原文和事实问题，不含修改建议，通过持久任务检查点按顺序发送纯文字，不生成或回传 Word、PPT 附件。
