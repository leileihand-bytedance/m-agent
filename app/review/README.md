...
5. **多文件联合审核**：用户明确说“联合审核”后连续发送 2 至 5 份 `.docx`，再说“开始审核”；逐份执行原有内容审核，并检查正文与其他文件的一致性。
...
企业微信审核 Bot
  ...
  -> 用户明确要求联合审核: intake.py + multi_file_reviewer.py
...
1. 用户先说“联合审核”“多文件审核”或“把正文和附件一起核对”。也可以先发第一份文件，再补说联合审核，系统会把刚发文件纳入本次任务。
2. 用户连续发送 2 至 5 份 `.docx`，可在中途补充审核重点。
3. 文件发完后回复“开始审核”。
4. 系统只在文件名证据唯一时自动识别主文件，例如一份普通文件配多份名称含“附件1”“附件2”的文件。
5. **系统不会把第一份上传文件默认当成正文。** 无法可靠判断时，Bot 会列出文件并要求用户回复“第2个是正文”或直接写主文件名。

若希望后续文件不被单独审核，应在发送这些文件前先说“联合审核”；进入联合审核模式后，文件只暂存，不会逐份立即启动独立任务。未进入联合模式前直接发送的第一份文件仍可能已经启动普通单文件审核。
...
- 待组装状态默认保留 30 分钟，持久化在 `M-Agent-Files/runtime/intake/review/`，按入口和用户隔离，Bot 重启后可恢复。
...
- 当前组装逻辑位于旧审核模块；跨写作和审核复用的底座级公共接口仍由 `TODO-003` 承接。
