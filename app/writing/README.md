...
- “这一段只保留一个案例”“把新数据补到数字金融那篇第二段”“沿用上一版结构另写一份”等自然表达先经过公共任务关系层。任务序号、明确标题或“这个直报/简报”等文种指向优先；“再改、继续改、接着改”默认承接用户当前稿，不能被旧稿的主题关键词抢走。确实无法唯一定位时只追问一个区分问题；用户回复稿件名称或纠正“另写一份”后沿用已上传材料，不要求重新发送。
