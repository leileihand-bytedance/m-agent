...
企业微信写作 Bot
  -> app/writing/bot.py
  -> app/writing/task_execution.py # 直报/简报/深银协动态/内参周报持久任务适配
...
- 单素材和多素材简报在生成前先完成九类题材判断、语义策划和事实台账选择，生成后检查 1200 字上限、数字依据和高风险强结论。续改会先解析修改范围；只改标题或指定段落时，由代码恢复未点名内容，避免模型顺手重写全文。
...
- 内参周报不要求用户提供素材。用户发送“生成本周内参周报”后，入口把独立 `internal_weekly` Skill 提交到写作持久队列；周一 15:00 收盘前会先生成上周五板块内容，并在资本市场固定位置用红色粗体标注“今日资本市场内容待收盘后更新”，15:00 后生成上周加当日完整版本。用户也可发送“生成一下今天的资本市场综述”，收盘后只生成可替换的当日行情更新块。第一阶段回传简短完成说明、带原文链接和核验信息的 Markdown 内容核对稿以及 JSON 溯源清单，不生成 Word。其板块规则和信源表属于 Skill 自身，不调用审核模块；同一大会或论坛的互补正式成果合并为一条，监管动态以实际监管机构为主体；同业动向按境内民营/数字银行、国际及香港数字银行、银行科技子公司分组检索，机构官网、投资者关系和法定披露优先，达到统一评分才入选、最多 5 条且排除营销宣传；市场观察在固定资本市场综述后按七类主题独立检索影响增长、通胀、利率、汇率、流动性、风险偏好或银行资产负债环境的国内外大事，按统一评分达标才入选、最多 5 条且不凑数；前沿观点依据可逐字核验的研报原文生成中文压缩摘要并在结尾标明出处，资本市场综述缺少任一固定数据组时不让模型补齐。
- 企业微信新直报、简报、深银协动态和内参周报先返回对应写作类型的已受理提示，再由后台 worker 生成并主动发送结果。主动文字使用企业微信 `aibot_send_msg` 支持的 Markdown 类型，附件使用媒体消息；简报正式 Word 按“标题正文文字 + 附件”两个交付项保存检查点，内参周报按“完成说明 + Markdown 核对稿 + JSON 溯源清单”三个交付项保存检查点。后台始终保留任务编号，但正常受理和重复提交提示不向用户展示。任务入队前已经创建正式写作 job、复制文件快照；处理、会话收尾和发送分别记录检查点，Bot 重启后不会因为 intake 临时文件丢失而无法恢复。文本和附件逐项保存“已确认送达、明确未送达、送达未知”及判断依据；已经确认的部分不会重复发送，未知状态暂停自动重发并交由本机管理员恢复。
- 写作队列使用独立 `runtime/task-execution/writing.sqlite3`，默认 1 个 worker、同一用户同一时刻只跑 1 项任务；不同用户按 `userid` 隔离并由全局 worker 上限调度。审核队列与写作队列分离，两个 worker 不会互相领取任务。直报、统一简报、深银协动态和内参周报使用持久队列，综合调研当前仍为实时执行。
...
- “这一段只保留一个案例”“把新数据补到数字金融那篇第二段”“沿用上一版结构另写一份”等自然表达先经过公共任务关系层。任务序号、明确标题或“这个直报/简报”等文种指向优先；“再改、继续改、接着改”默认承接用户当前稿，不能被旧稿的主题关键词抢走。公共层确定具体任务后，简报 Skill 再把改稿要求解析成标题、指定段落或全文范围。确实无法唯一定位时只追问一个区分问题；用户回复稿件名称或纠正“另写一份”后沿用已上传材料，不要求重新发送。
