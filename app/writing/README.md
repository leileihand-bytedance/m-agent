...
- 内参周报不要求用户提供素材。用户发送“生成本周内参周报”后，入口把独立 `internal_weekly` Skill 提交到写作持久队列，并明确提示该任务需要检索、核验多类来源，耗时明显长于普通稿件；周一 15:00 收盘前会先生成上周五板块内容，并在资本市场固定位置用红色粗体标注“今日资本市场内容待收盘后更新”，15:00 后生成上周加当日完整版本。用户也可发送“生成一下今天的资本市场综述”，收盘后只生成可替换的当日行情更新块。初次只回传简短完成说明和精简 Markdown 内容核对稿：顶部显示出版日与统计期，每条显示正文及一行原文链接，确有异常时才显示待核事项；JSON 结构化溯源记录、来源原句、正文哈希和版本状态只在任务后台保留。只有用户明确确认当前版本核对无误并要求生成 Word 时，Skill 才从净化母版生成洁净版 `.docx`，不重新搜索或调用模型。普通“生成 Word”会先要求用户明确批准。母版保留水印、红线、页眉页脚、页码和自动目录域；后台 Microsoft Word 在任务交付前更新整个目录并保存，代码核对目录标题、页码和书签完整后才返回附件，用户打开成品无需再手工更新目录。其板块规则和信源表属于 Skill 自身，不调用审核模块；同一大会或论坛的互补正式成果合并为一条，监管动态以实际监管机构为主体；同业动向按境内民营/数字银行、国际及香港数字银行、银行科技子公司分组检索，机构官网、投资者关系和法定披露优先，达到统一评分才入选、最多 5 条且排除营销宣传；市场观察在固定资本市场综述后按七类主题独立检索影响增长、通胀、利率、汇率、流动性、风险偏好或银行资产负债环境的国内外大事，按统一评分达标才入选、最多 5 条且不凑数；前沿观点依据可逐字核验的研报原文生成中文压缩摘要并在结尾标明出处，资本市场综述缺少任一固定数据组时不让模型补齐。
- 完整内参周报在一个持久队列任务内部并行运行党政要闻、监管动态、同业动向、市场观察和前沿观点五个执行单元，并发上限为 5；完成后按固定顺序确定性汇总。单个板块失败只把该板块降级为待核，不取消其他板块，也不建立五个独立队列任务。单独更新今日资本市场综述仍只运行市场观察更新链路。
- 企业微信新直报、简报、综合调研、深银协动态和内参周报先返回对应写作类型的已受理提示，再由后台 worker 生成并主动发送结果。主动文字使用企业微信 `aibot_send_msg` 支持的 Markdown 类型，附件使用媒体消息；简报正式 Word 和综合调研分别按“完成文字 + Word 附件”两个交付项保存检查点；内参周报初次生成按“完成说明 + Markdown 核对稿”保存，内部 JSON 不进入交付队列，批准后 Word 按“完成说明 + `.docx` 附件”保存。后台始终保留任务编号，但正常受理和重复提交提示不向用户展示。任务入队前已经创建正式写作 job、复制文件快照；处理、会话收尾和发送分别记录检查点，Bot 重启后不会因为 intake 临时文件丢失而无法恢复。文本和附件逐项保存“已确认送达、明确未送达、送达未知”及判断依据；已经确认的部分不会重复发送，未知状态暂停自动重发并交由本机管理员恢复。普通运行日志额外记录任务、交付项类型和状态，不记录正文、文件名、路径或用户 ID。
