...
## 一一六、[2026-06-25] 审核消息文案优化

### 改动

移除面向用户的"低级错误/内容质量"表述，改为更简洁的格式：

| 位置 | 改动前 | 改动后 |
|------|--------|--------|
| 收到文件 | "已收到文件,在努力审核了,请稍等……" | "收到文件啦，正在加紧审核，请稍等（模型反应有点慢，你可以先干点别的，一会儿再来看）……" |
| 第一阶段 | "第一阶段审核完成（低级错误）" + "格式检查 + 基础内容审核，共 N 条" | "第一阶段审核结果如下：" + "共 N 条：" |
| 第二阶段 | "第二阶段审核完成（内容质量）" + "深度内容审核，共 N 条" | "第二阶段审核结果如下：" + "共 N 条：" |

### 文件改动

- `app/review/main.py` — 收到文件提示文案
- `app/review/output_formatter.py` — phase1/phase2 标题和描述文案

---

## 一一五、[2026-06-25] content-wrong-section 误报优化

### 问题

最近一次审核（20260625-003）发现两类误报：
1. **引用语境误报**：段49正文引用"中国人民银行"数据，被判为内容主体
2. **重复报案**：段38、段39是同一篇新闻（国务院/中央纪委），分别报错

### 修复

**标题优先原则**：只检测新闻标题段落（5-60字符，无句末标点），正文跟随标题不再单独检测。

**引用语境排除**：实体出现在"据X数据显示"、"X公布的数据"等模式中时，不作为主体判断依据。

```python
def _is_entity_in_citation_context(text, entity) -> bool:
    # 实体前有"据/根据/来自/依据"前缀，或
    # 实体后有"数据显示/公布的数据"后缀 → 视为引用，不作主体
```

**新增辅助函数**：`check_section_mismatch()` 新增 `_is_news_title()` 和 `_is_entity_in_citation_context()`。
...
- `app/review/reviewer.py` — `check_section_mismatch()` 重构，`_is_news_title()` 和 `_is_entity_in_citation_context()` 新增
- `app/review/section_entities.py` — 新增 `MARKET_OBSERVATION_MARKERS`，支持A股/港股/美股综述等专属内容类型检测
- `tests/test_section_classifier.py` — 新增4个测试用例（共10个）
- `STATUS-REPORT.md` — 本条
